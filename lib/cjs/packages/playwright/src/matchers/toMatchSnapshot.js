...
const comparators = require('../../../playwright-core/src/server/utils/comparators.js');
require('node:crypto');
...
class SnapshotHelper {
  constructor(testInfo, matcherName, locator, anonymousSnapshotExtension, configOptions, nameOrOptions, optOptions) {
    ...
    this.legacyExpectedPath = util.addSuffixToFilePath(outputBasePath, "-expected");
    this.previousPath = util.addSuffixToFilePath(outputBasePath, "-previous");
    this.actualPath = util.addSuffixToFilePath(outputBasePath, "-actual");
    this.diffPath = util.addSuffixToFilePath(outputBasePath, "-diff");
    ...
    this.mimeType = utilsBundle.mime.getType(path.basename(this.expectedPath)) ?? "application/octet-stream";
    this.comparator = comparators.getComparator(this.mimeType);
  ...
  handleMatchingNegated() {
    const message = [
      utilsBundle.colors.red(`${this.kind} comparison failed:`),
      "",
  ...
  handleMissing(actual, step) {
    ...
    step?._attachToStep({ name: util.addSuffixToFilePath(this.attachmentBaseName, "-expected"), contentType: this.mimeType, path: this.expectedPath });
    writeFileSync(this.actualPath, actual);
    step?._attachToStep({ name: util.addSuffixToFilePath(this.attachmentBaseName, "-actual"), contentType: this.mimeType, path: this.actualPath });
  ...
  handleDifferent(actual, expected, previous, diff, header, diffError, log, step) {
    ...
    if (previous !== void 0) {
      ...
      step?._attachToStep({ name: util.addSuffixToFilePath(this.attachmentBaseName, "-previous"), contentType: this.mimeType, path: this.previousPath });
    }
    if (actual !== void 0) {
      writeFileSync(this.actualPath, actual);
      step?._attachToStep({ name: util.addSuffixToFilePath(this.attachmentBaseName, "-actual"), contentType: this.mimeType, path: this.actualPath });
    ...
    if (diff !== void 0) {
      ...
      step?._attachToStep({ name: util.addSuffixToFilePath(this.attachmentBaseName, "-diff"), contentType: this.mimeType, path: this.diffPath });
    }
    if (log?.length)
      output.push(util.callLogText(log));
    else
      output.push("");
...
function toMatchSnapshot(received, nameOrOptions = {}, optOptions = {}) {
  ...
  if (this.isNot) {
    if (!fs.default.existsSync(helper.expectedPath))
      return helper.handleMissingNegated();
    const isDifferent = !!helper.comparator(received, fs.default.readFileSync(helper.expectedPath), helper.options);
    return isDifferent ? helper.handleDifferentNegated() : helper.handleMatchingNegated();
  }
  if (!fs.default.existsSync(helper.expectedPath))
    return helper.handleMissing(received, this._stepInfo);
  const expected = fs.default.readFileSync(helper.expectedPath);
  if (helper.updateSnapshots === "all") {
    if (!comparators.compareBuffersOrStrings(received, expected))
  ...
  if (!result)
    return helper.handleMatching();
  const receiver = stringUtils.isString(received) ? "string" : "Buffer";
  const header = matcherHint.matcherHint(this, void 0, "toMatchSnapshot", receiver, void 0, void 0);
...
async function toHaveScreenshot(pageOrLocator, nameOrOptions = {}, optOptions = {}) {
  ...
  util.expectTypes(pageOrLocator, ["Page", "Locator"], "toHaveScreenshot");
  ...
  const hasSnapshot = fs.default.existsSync(helper.expectedPath);
  if (this.isNot) {
    if (!hasSnapshot)
      return helper.handleMissingNegated();
    expectScreenshotOptions.expected = await fs.default.promises.readFile(helper.expectedPath);
  ...
  if (!hasSnapshot) {
    ...
    if (errorMessage2) {
      const header2 = matcherHint.matcherHint(this, locator, "toHaveScreenshot", receiver, void 0, void 0, timedOut2 ? timeout : void 0);
  ...
  const expected = await fs.default.promises.readFile(helper.expectedPath);
  ...
  if (!errorMessage) {
    if (helper.updateSnapshots === "all" && actual && comparators.compareBuffersOrStrings(actual, expected)) {
      console.log(helper.expectedPath + " is re-generated, writing actual.");
      return writeFiles();
    }
    return helper.handleMatching();
  ...
  if (helper.updateSnapshots === "changed" || helper.updateSnapshots === "all")
    return writeFiles();
  const header = matcherHint.matcherHint(this, void 0, "toHaveScreenshot", receiver, void 0, void 0, timedOut ? timeout : void 0);
  return helper.handleDifferent(actual, expectScreenshotOptions.expected, previous, diff, header, errorMessage, log, this._stepInfo);
}
function writeFileSync(aPath, content) {
  fs.default.mkdirSync(path.dirname(aPath), { recursive: true });
  fs.default.writeFileSync(aPath, content);
}
...
async function loadScreenshotStyles(stylePath) {
  ...
  const styles = await Promise.all(stylePaths.map(async (stylePath2) => {
    const text = await fs.default.promises.readFile(stylePath2, "utf8");
    return text.trim();
  }));
  return styles.join("\n").trim() || void 0;
}
