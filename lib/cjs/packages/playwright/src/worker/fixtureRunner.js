...
class Fixture {
  constructor(runner, registration) {
    ...
    const isUserFixture = this.registration.location && util.filterStackFile(this.registration.location.file);
  ...
  async _setupInternal(testInfo) {
    ...
    let called = false;
    const useFuncStarted = new manualPromise.ManualPromise();
    const useFunc = async (value) => {
      if (called)
        throw new Error(`Cannot provide fixture value for the second time`);
      called = true;
      this.value = value;
      this._useFuncFinished = new manualPromise.ManualPromise();
...
function getRequiredFixtureNames(fn, location) {
  return fixtures.fixtureParameterNames(fn, location ?? { file: "<unknown>", line: 1, column: 1 }, (e) => {
    throw new Error(`${util.formatLocation(e.location)}: ${e.message}`);
  });
}
