...
class TestTracing {
  ...
  async startIfNeeded(value) {
    ...
    if (!this._liveTraceFile && this._options._live) {
      this._liveTraceFile = { file: path.join(this._tracesDir, `${this._testInfo.testId}-test.trace`), fs: new fileUtils.SerializedFS() };
  ...
  _generateNextTraceRecordingPath() {
    const file = path.join(this._artifactsDir, crypto.createGuid() + ".zip");
  ...
  async stopIfNeeded() {
    ...
    if (this._shouldAbandonTrace()) {
      for (const file of this._temporaryTraceFiles)
        await fs.default.promises.unlink(file).catch(() => {
        });
      return;
    }
    ...
    if (this._options?.sources) {
      ...
      for (const sourceFile of sourceFiles) {
        await fs.default.promises.readFile(sourceFile, "utf8").then((source) => {
          zipFile.addBuffer(Buffer.from(source), "resources/src@" + crypto.calculateSha1(sourceFile) + ".txt");
        }).catch(() => {
        });
      }
    }
    ...
    for (const event of this._traceEvents.filter((e) => e.type === "after")) {
      for (const attachment of event.attachments || []) {
        let contentPromise;
        if (attachment.path)
          contentPromise = fs.default.promises.readFile(attachment.path).catch(() => void 0);
        else if (attachment.base64)
          contentPromise = Promise.resolve(Buffer.from(attachment.base64, "base64"));
        const content = await contentPromise;
        if (content === void 0)
          continue;
        const sha1 = crypto.calculateSha1(content);
    ...
    await new Promise((f) => {
      zipFile.end(void 0, () => {
        zipFile.outputStream.pipe(fs.default.createWriteStream(this._generateNextTraceRecordingPath())).on("close", f);
      });
    });
  ...
  appendForError(error) {
    ...
    const stack = rawStack ? util.filteredStackTrace(rawStack) : [];
  ...
  appendBeforeActionForStep(callId, parentId, options) {
    this._appendTraceEvent({
      ...
      class: "Test",
      method: "step",
      title: util.stepTitle(options.category, options.title),
...
async function mergeTraceFiles(fileName, temporaryTraceFiles) {
  temporaryTraceFiles = temporaryTraceFiles.filter((file) => fs.default.existsSync(file));
  if (temporaryTraceFiles.length === 1) {
    await fs.default.promises.rename(temporaryTraceFiles[0], fileName);
    return;
  }
  const mergePromise = new manualPromise.ManualPromise();
  const zipFile = new zipBundle.yazl.ZipFile();
  ...
  zipFile.end(void 0, () => {
    zipFile.outputStream.pipe(fs.default.createWriteStream(fileName)).on("close", () => {
      void Promise.all(temporaryTraceFiles.map((tempFile) => fs.default.promises.unlink(tempFile))).then(() => {
...
exports.testTraceEntryName = testTraceEntryName;
