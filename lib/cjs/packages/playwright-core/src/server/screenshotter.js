...
class Screenshotter {
  ...
  async screenshotElement(progress, handle, options) {
    ...
    return this._queue.postTask(async () => {
      ...
      assert.assert(boundingBox, "Node is either not visible or not an HTMLElement");
      assert.assert(boundingBox.width !== 0, "Node has 0 width.");
      assert.assert(boundingBox.height !== 0, "Node has 0 height.");
      const fitsViewport = boundingBox.width <= viewportSize.width && boundingBox.height <= viewportSize.height;
      const scrollOffset = await this._page.mainFrame().waitForFunctionValueInUtility(progress, () => ({ x: window.scrollX, y: window.scrollY }));
      const documentRect = { ...boundingBox };
      documentRect.x += scrollOffset.x;
      documentRect.y += scrollOffset.y;
      const buffer = await this._screenshot(progress, format, helper.helper.enclosingIntRect(documentRect), void 0, fitsViewport, options);
      await this._restorePageAfterScreenshot();
      return buffer;
    });
  }
  ...
  async _maskElements(progress, options) {
    ...
    const framesToParsedSelectors = new multimap.MultiMap();
    const cleanup = async () => {
...
function trimClipToSize(clip, size) {
  ...
  assert.assert(result.width && result.height, "Clipped area is either empty or outside the resulting image");
  return result;
}
function validateScreenshotOptions(options) {
  let format = null;
  if (options.type) {
    assert.assert(options.type === "png" || options.type === "jpeg", "Unknown options.type value: " + options.type);
    format = options.type;
  }
  if (!format)
    format = "png";
  if (options.quality !== void 0) {
    assert.assert(format === "jpeg", "options.quality is unsupported for the " + format + " screenshots");
    assert.assert(typeof options.quality === "number", "Expected options.quality to be a number but found " + typeof options.quality);
    assert.assert(Number.isInteger(options.quality), "Expected options.quality to be an integer");
    assert.assert(options.quality >= 0 && options.quality <= 100, "Expected options.quality to be between 0 and 100 (inclusive), got " + options.quality);
  }
  if (options.clip) {
    assert.assert(typeof options.clip.x === "number", "Expected options.clip.x to be a number but found " + typeof options.clip.x);
    assert.assert(typeof options.clip.y === "number", "Expected options.clip.y to be a number but found " + typeof options.clip.y);
    assert.assert(typeof options.clip.width === "number", "Expected options.clip.width to be a number but found " + typeof options.clip.width);
    assert.assert(typeof options.clip.height === "number", "Expected options.clip.height to be a number but found " + typeof options.clip.height);
    assert.assert(options.clip.width !== 0, "Expected options.clip.width not to be 0.");
    assert.assert(options.clip.height !== 0, "Expected options.clip.height not to be 0.");
  }
  return format;
}
