...
const harRecorder = require('./har/harRecorder.js');
...
const storageScriptSource = require('../generated/storageScriptSource.js');

class BrowserContext extends instrumentation.SdkObject {
  constructor(browser, options, browserContextId) {
    ...
    this._selectors = new selectors.Selectors(options.selectorEngines || [], options.testIdAttributeName);
    this.fetchRequest = new fetch.BrowserContextAPIRequestContext(this);
    this.tracing = new tracing.Tracing(this, browser.options.tracesDir);
    this.clock = new clock.Clock(this);
    this.dialogManager = new dialog.DialogManager(this.instrumentation);
  }
  static {
    this.Events = {
      Console: "console",
  ...
  async _initialize() {
    ...
    if (debug.debugMode() === "inspector")
      await recorderApp.RecorderApp.show(this, { pauseOnNextStatement: true });
    if (this._debugger.isPaused())
      recorderApp.RecorderApp.showInspectorNoReply(this);
    this._debugger.on(_debugger.Debugger.Events.PausedStateChanged, () => {
      if (this._debugger.isPaused())
        recorderApp.RecorderApp.showInspectorNoReply(this);
    });
    if (debug.debugMode() === "console")
...
`);
  ...
  async _ensureVideosPath() {
    if (this._options.recordVideo)
      await fileUtils.mkdirIfNeeded(path.join(this._options.recordVideo.dir, "dummy"));
  }
  canResetForReuse() {
  ...
  async exposePlaywrightBindingIfNeeded() {
    ...
    this.bindingsInitScript = page.PageBinding.createInitScript();
  ...
  async exposeBinding(progress, name, needsHandle, playwrightBinding, forClient) {
    ...
    const binding = new page.PageBinding(name, playwrightBinding, needsHandle);
...
`).join("");
  ...
  async _loadDefaultContextAsIs(progress) {
    if (!this.possiblyUninitializedPages().length) {
      const waitForEvent = helper.helper.waitForEvent(progress, this, BrowserContext.Events.Page);
  ...
  async addInitScript(progress, source) {
    const initScript = new page.InitScript(source);
  ...
  async _deleteAllTempDirs() {
    await Promise.all(this._tempDirs.map(async (dir) => await fs.default.promises.unlink(dir).catch((e) => {
    })));
  }
  ...
  async newPage(progress, isServerSide) {
    const page$1 = await progress.raceWithCleanup(this.doCreateNewPage(isServerSide), (page2) => page2.close());
    const pageOrError = await progress.race(page$1.waitForInitializedOrError());
    if (pageOrError instanceof page.Page) {
  ...
  async extendInjectedScript(source, arg) {
    ...
    const installInPage = (page$1) => {
      page$1.on(page.Page.Events.InternalFrameNavigatedToNewDocument, installInFrame);
      return Promise.all(page$1.frames().map(installInFrame));
    };
  ...
  harStart(page, options) {
    const harId = crypto.createGuid();
    this._harRecorders.set(harId, new harRecorder.HarRecorder(this, page, options));
    return harId;
  }
