...
class WSServer {
  ...
  async listen(port = 0, hostname, path) {
    debugLogger.debugLogger.log("server", `Server started at ${/* @__PURE__ */ new Date()}`);
    const server = network.createHttpServer(this._delegate.onRequest);
    server.on("error", (error) => debugLogger.debugLogger.log("server", String(error)));
    this.server = server;
    ...
    debugLogger.debugLogger.log("server", "Listening at " + wsEndpoint);
    this._wsServer = new utilsBundle.wsServer({
      noServer: true,
      perMessageDeflate
    });
...
`);
    ...
    this._wsServer.on("connection", (ws, request) => {
      debugLogger.debugLogger.log("server", "Connected client ws.extension=" + ws.extensions);
      const url = new URL("http://localhost" + (request.url || ""));
      const id = String(++lastConnectionId);
      debugLogger.debugLogger.log("server", `[${id}] serving connection: ${request.url}`);
  ...
  async close() {
    ...
    if (!server)
      return;
    debugLogger.debugLogger.log("server", "closing websocket server");
    ...
    await waitForClose;
    debugLogger.debugLogger.log("server", "closing http server");
    if (this.server)
      await new Promise((f) => this.server.close(f));
    this._wsServer = void 0;
    this.server = void 0;
    debugLogger.debugLogger.log("server", "closed server");
  }
}
