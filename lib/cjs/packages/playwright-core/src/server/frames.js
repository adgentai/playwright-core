...
const fileUploadUtils = require('./fileUploadUtils.js');
const frameSelectors = require('./frameSelectors.js');
...
class FrameManager {
  ...
  frameAttached(frameId, parentFrameId) {
    ...
    if (!parentFrame) {
      if (this._mainFrame) {
        ...
        assert.assert(!this._frames.has(frameId));
      ...
      this._page.emit(page.Page.Events.FrameAttached, frame);
      return frame;
    }
  }
  async waitForSignalsCreatedBy(progress, waitAfter, action) {
    ...
    await new Promise(task.makeWaitForNextTask());
    return result;
  }
  ...
  frameCommittedNewDocumentNavigation(frameId, url, name, documentId, initial) {
    ...
    if (!initial) {
      debugLogger.debugLogger.log("api", `  navigated to "${url}"`);
  ...
  requestStarted(request, route) {
    ...
    this._page.emitOnContext(browserContext.BrowserContext.Events.Request, request);
    if (route)
  ...
  requestReceivedResponse(response) {
    if (response.request()._isFavicon)
      return;
    this._page.emitOnContext(browserContext.BrowserContext.Events.Response, response);
  }
  reportRequestFinished(request, response) {
    this._inflightRequestFinished(request);
    if (request._isFavicon)
      return;
    this._page.emitOnContext(browserContext.BrowserContext.Events.RequestFinished, { request, response });
  }
  requestFailed(request, canceled) {
    ...
    if (request._isFavicon)
      return;
    this._page.emitOnContext(browserContext.BrowserContext.Events.RequestFailed, request);
  }
  ...
  _removeFramesRecursively(frame) {
    ...
    if (!this._page.isClosed())
      this._page.emit(page.Page.Events.FrameDetached, frame);
  }
  ...
  onWebSocketRequest(requestId) {
    ...
    if (ws && ws.markAsNotified())
      this._page.emit(page.Page.Events.WebSocket, ws);
  }
...
class Frame extends instrumentation.SdkObject {
  constructor(page, id, parentFrame) {
    ...
    this._detachedScope = new manualPromise.LongStandingScope();
    ...
    this.selectors = new frameSelectors.FrameSelectors(this);
    this._contextData.set("main", { contextPromise: new manualPromise.ManualPromise(), context: null });
    this._contextData.set("utility", { contextPromise: new manualPromise.ManualPromise(), context: null });
  ...
  _onLifecycleEvent(event) {
    ...
    if (this === this._page.mainFrame() && this._url !== "about:blank")
      debugLogger.debugLogger.log("api", `  "${event}" event fired`);
    this._page.mainFrame()._recalculateNetworkIdle();
  }
  _onClearLifecycle() {
  ...
  _recalculateNetworkIdle(frameThatAllowsRemovingNetworkIdle) {
    ...
    if (isNetworkIdle && !this._firedLifecycleEvents.has("networkidle")) {
      ...
      if (this === this._page.mainFrame() && this._url !== "about:blank")
        debugLogger.debugLogger.log("api", `  "networkidle" event fired`);
    }
  ...
  async raceNavigationAction(progress, action) {
    return manualPromise.LongStandingScope.raceMultiple([
      this._detachedScope,
  ...
  async goto(progress, url, options = {}) {
    const constructedNavigationURL = urlMatch.constructURLBasedOnBaseURL(this._page.browserContext._options.baseURL, url);
  ...
  async gotoImpl(progress, url, options) {
    ...
    url = helper.helper.completeUserURL(url);
    ...
    if (navigateResult.newDocumentId) {
      ...
      if (events.length)
        ...
        event = await helper.helper.waitForEvent(progress, this, Frame.Events.InternalNavigation, predicate).promise;
    ...
    if (!this._firedLifecycleEvents.has(waitUntil))
      await helper.helper.waitForEvent(progress, this, Frame.Events.AddLifecycle, (e) => e === waitUntil).promise;
  ...
  async _waitForNavigation(progress, requiresNewDocument, options) {
    ...
    const navigationEvent = await helper.helper.waitForEvent(progress, this, Frame.Events.InternalNavigation, (event) => {
      if (event.error)
        return true;
  ...
  _context(world) {
    return this._contextData.get(world).contextPromise.then((contextOrDestroyedReason) => {
      if (contextOrDestroyedReason instanceof javascript.ExecutionContext)
  ...
  async querySelector(selector, options) {
    debugLogger.debugLogger.log("api", `    finding element using the selector "${selector}"`);
  ...
  async setContent(progress, html, options) {
    await this.raceNavigationAction(progress, async () => {
      ...
      const tagPromise = new manualPromise.ManualPromise();
  ...
  async _raceWithCSPError(func) {
    ...
    const errorPromise = new Promise((resolve) => {
      listeners.push(eventsHelper.eventsHelper.addEventListener(this._page.browserContext, browserContext.BrowserContext.Events.Console, (message) => {
  ...
  async retryWithProgressAndTimeouts(progress, timeouts, action) {
    ...
    while (true) {
      ...
      if (timeout) {
        ...
        await progress.race(manualPromise.LongStandingScope.raceMultiple([
          this._page.openScope,
          this._detachedScope
        ], actionPromise));
      }
  ...
  isNonRetriableError(e) {
    if (progress.isAbortError(e))
      return true;
    if (javascript.isJavaScriptErrorInEvaluate(e) || protocolError.isSessionClosedError(e))
      return true;
    if (dom.isNonRecoverableDOMError(e) || cssParser.isInvalidSelectorError(e))
      return true;
    if (this.isDetached())
      return true;
    return false;
  ...
  async generateLocatorString(progress, selector) {
    ...
    return locatorGenerators.asLocator(this._page.browserContext._browser.sdkLanguage(), result.reverse().join(" >> internal:control=enter-frame >> "));
  }
  ...
  async setInputFiles(progress, selector, params) {
    const inputFileItems = await fileUploadUtils.prepareFilesForUpload(this, params);
  ...
  async expect(progress, selector, options, timeout, errorResultHandler) {
    progress.log(`${protocolFormatter.renderTitleForCall(progress.metadata)}${timeout ? ` with timeout ${timeout}ms` : ""}`);
  ...
  async _expectImpl(progress, selector, options, errorResultHandler) {
    ...
    const handleError = (e, isProgressError) => {
      if (javascript.isJavaScriptErrorInEvaluate(e) || cssParser.isInvalidSelectorError(e))
        throw e;
      const result = { matches: options.isNot, log: callLog.compressCallLog(progress.metadata.log) };
  ...
  async waitForFunctionExpression(progress, expression, isFunction, arg, options, world = "main") {
    if (typeof options.pollingInterval === "number")
      assert.assert(options.pollingInterval > 0, "Cannot poll with non-positive interval: " + options.pollingInterval);
    expression = javascript.normalizeEvaluationExpression(expression, isFunction);
  ...
  _setContext(world, context) {
    ...
    if (context)
      ...
      data.contextPromise = new manualPromise.ManualPromise();
  }
  ...
  _startNetworkIdleTimer() {
    assert.assert(!this._networkIdleTimer);
  ...
  _asLocator(selector) {
    return locatorGenerators.asLocator(this._page.browserContext._browser.sdkLanguage(), selector);
  }
}
class SignalBarrier {
  constructor(progress) {
    this._protectCount = 0;
    this._promise = new manualPromise.ManualPromise();
  ...
  addFrameNavigation(frame) {
    if (frame.parentFrame())
      return;
    this.retain();
    const waiter = helper.helper.waitForEvent(this._progress, frame, Frame.Events.InternalNavigation, (e) => {
      if (!e.isPublic)
        return false;
    ...
    manualPromise.LongStandingScope.raceMultiple([
