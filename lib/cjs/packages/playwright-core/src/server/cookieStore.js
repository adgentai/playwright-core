...
class Cookie {
  ...
  matches(url) {
    if (this._raw.secure && (url.protocol !== "https:" && !network.isLocalHostname(url.hostname)))
      return false;
      ...
      case "expires":
        ...
        if (isFinite(expiresMs)) {
          if (expiresMs <= 0)
            ...
            cookie.expires = Math.min(expiresMs / 1e3, network.kMaxCookieExpiresDateInSeconds);
        }
        break;
      case "max-age":
        ...
        if (isFinite(maxAgeSec)) {
          if (maxAgeSec <= 0)
            ...
            cookie.expires = Math.min(Date.now() / 1e3 + maxAgeSec, network.kMaxCookieExpiresDateInSeconds);
        }
        break;
      case "domain":
