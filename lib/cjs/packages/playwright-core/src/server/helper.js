...
class Helper {
  ...
  static waitForEvent(progress, emitter, event, predicate) {
    ...
    const promise = new Promise((resolve, reject) => {
      listeners.push(eventsHelper.eventsHelper.addEventListener(emitter, event, (eventArg) => {
        try {
    ...
    const dispose = () => eventsHelper.eventsHelper.removeEventListeners(listeners);
  ...
  static debugProtocolLogger(protocolLogger) {
    return (direction, message) => {
      ...
      if (debugLogger.debugLogger.isEnabled("protocol")) {
        let text = JSON.stringify(message);
        if (text.length > MAX_LOG_LENGTH)
          text = text.substring(0, MAX_LOG_LENGTH / 2) + " <<<<<( LOG TRUNCATED )>>>>> " + text.substring(text.length - MAX_LOG_LENGTH / 2);
        debugLogger.debugLogger.log("protocol", (direction === "send" ? "SEND ► " : "◀ RECV ") + text);
      }
    };
  }
