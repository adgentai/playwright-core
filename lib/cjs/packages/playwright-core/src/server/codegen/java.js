...
class JavaLanguageGenerator {
  ...
  generateAction(actionInContext) {
    ...
    const formatter = new javascript.JavaScriptFormatter(offset);
  ...
  _asLocator(selector, inFrameLocator) {
    return locatorGenerators.asLocator("java", selector, inFrameLocator);
  }
  generateHeader(options) {
    const formatter = new javascript.JavaScriptFormatter();
    if (this._mode === "junit") {
      formatter.add(`
