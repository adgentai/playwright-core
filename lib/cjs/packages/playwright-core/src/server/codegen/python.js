...
class PythonLanguageGenerator {
  ...
  generateAction(actionInContext) {
    ...
    const signals = language.toSignalMap(action);
    if (signals.dialog)
    ...
    switch (action.name) {
      ...
      case "click": {
        ...
        if (action.clickCount === 2)
          method = "dblclick";
        const options = language.toClickOptionsForSourceCode(action);
  ...
  _asLocator(selector) {
    return locatorGenerators.asLocator("python", selector);
  }
...
function formatOptions(value, hasArguments, asDict) {
  ...
  return (hasArguments ? ", " : "") + keys.map((key) => {
    if (asDict)
      return `"${stringUtils.toSnakeCase(key)}": ${formatValue(value[key])}`;
    return `${stringUtils.toSnakeCase(key)}=${formatValue(value[key])}`;
  }).join(", ");
}
function formatContextOptions(options, deviceName, asDict) {
  options = { ...options, recordHar: void 0 };
  const device = deviceName && deviceDescriptors.deviceDescriptors[deviceName];
  if (!device)
    return formatOptions(options, false, asDict);
  return `**playwright.devices[${quote(deviceName)}]` + formatOptions(language.sanitizeDeviceOptions(device, options), true, asDict);
}
