...
class HarTracer {
  constructor(context, page, delegate, options) {
    ...
    this._baseURL = context instanceof fetch.APIRequestContext ? context._defaultOptions().baseURL : context._options.baseURL;
  }
  start(options) {
    if (this._started)
      return;
    ...
    this._started = true;
    const apiRequest = this._context instanceof fetch.APIRequestContext ? this._context : this._context.fetchRequest;
    this._eventListeners = [
      eventsHelper.eventsHelper.addEventListener(apiRequest, fetch.APIRequestContext.Events.Request, (event) => this._onAPIRequest(event)),
      eventsHelper.eventsHelper.addEventListener(apiRequest, fetch.APIRequestContext.Events.RequestFinished, (event) => this._onAPIRequestFinished(event))
    ];
    if (this._context instanceof browserContext.BrowserContext) {
      this._eventListeners.push(
        eventsHelper.eventsHelper.addEventListener(this._context, browserContext.BrowserContext.Events.Page, (page) => this._createPageEntryIfNeeded(page)),
        eventsHelper.eventsHelper.addEventListener(this._context, browserContext.BrowserContext.Events.Request, (request) => this._onRequest(request)),
        eventsHelper.eventsHelper.addEventListener(this._context, browserContext.BrowserContext.Events.RequestFinished, ({ request, response }) => this._onRequestFinished(request, response).catch(() => {
        })),
        eventsHelper.eventsHelper.addEventListener(this._context, browserContext.BrowserContext.Events.RequestFailed, (request) => this._onRequestFailed(request)),
        eventsHelper.eventsHelper.addEventListener(this._context, browserContext.BrowserContext.Events.Response, (response) => this._onResponse(response)),
        eventsHelper.eventsHelper.addEventListener(this._context, browserContext.BrowserContext.Events.RequestAborted, (request) => this._onRequestAborted(request)),
        eventsHelper.eventsHelper.addEventListener(this._context, browserContext.BrowserContext.Events.RequestFulfilled, (request) => this._onRequestFulfilled(request)),
        eventsHelper.eventsHelper.addEventListener(this._context, browserContext.BrowserContext.Events.RequestContinued, (request) => this._onRequestContinued(request))
      );
  ...
  _shouldIncludeEntryWithUrl(urlString) {
    return !this._options.urlFilter || urlMatch.urlMatches(this._baseURL, urlString, this._options.urlFilter);
  }
  ...
  _createPageEntryIfNeeded(page) {
    ...
    if (!pageEntry) {
      ...
      page.mainFrame().on(frames.Frame.Events.AddLifecycle, (event) => {
        if (event === "load")
  ...
  async _onRequestFinished(request, response) {
    ...
    const compressionCalculationBarrier = this._options.omitSizes ? void 0 : {
      _encodedBodySize: -1,
      _decodedBodySize: -1,
      barrier: new manualPromise.ManualPromise(),
      _check: function() {
    ...
    harEntry.timings.receive = response.request()._responseEndTiming !== -1 ? helper.helper.millisToRoundishMillis(response.request()._responseEndTiming - timing.responseStart) : -1;
    this._computeHarEntryTotalTime(harEntry);
    if (!this._options.omitSizes) {
  ...
  _storeResponseContent(buffer, content, resourceType) {
    ...
    if (this._options.content === "embed") {
      if (mimeType.isTextualMimeType(content.mimeType) && resourceType !== "font") {
      ...
      const sha1 = crypto.calculateSha1(buffer) + "." + (utilsBundle.mime.getExtension(content.mimeType) || "dat");
  ...
  _onResponse(response) {
    ...
    if (!this._options.omitTiming) {
      ...
      const dns = timing.domainLookupEnd !== -1 ? helper.helper.millisToRoundishMillis(timing.domainLookupEnd - timing.domainLookupStart) : -1;
      const connect = timing.connectEnd !== -1 ? helper.helper.millisToRoundishMillis(timing.connectEnd - timing.connectStart) : -1;
      const ssl = timing.connectEnd !== -1 ? helper.helper.millisToRoundishMillis(timing.connectEnd - timing.secureConnectionStart) : -1;
      const wait = timing.responseStart !== -1 ? helper.helper.millisToRoundishMillis(timing.responseStart - timing.requestStart) : -1;
      const receive = -1;
      harEntry.timings = {
        dns,
        connect,
  ...
  stop() {
    ...
    const context = this._context instanceof browserContext.BrowserContext ? this._context : void 0;
    const log = {
      version: "1.2",
      creator: {
        name: "Playwright",
        version: userAgent.getPlaywrightVersion()
      },
      browser: {
  ...
  _postDataForBuffer(postData, contentType, content) {
    ...
    if (content === "attach") {
      const sha1 = crypto.calculateSha1(postData) + "." + (utilsBundle.mime.getExtension(contentType) || "dat");
      if (this._options.includeTraceInfo)
        result._sha1 = sha1;
      else
        result._file = sha1;
...
function createHarEntry(method, url, frameref, options) {
  const harEntry = {
    ...
    _monotonicTime: options.includeTraceInfo ? time.monotonicTime() : void 0,
