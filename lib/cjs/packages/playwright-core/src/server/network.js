...
function rewriteCookies(cookies) {
  return cookies.map((c) => {
    assert.assert(c.url || c.domain && c.path, "Cookie should have a url or a domain/path pair");
    assert.assert(!(c.url && c.domain), "Cookie should have either url or domain");
    assert.assert(!(c.url && c.path), "Cookie should have either url or path");
    assert.assert(!(c.expires && c.expires < 0 && c.expires !== -1), "Cookie should have a valid expires, only -1 or a positive number for the unix timestamp in seconds is allowed");
    assert.assert(!(c.expires && c.expires > 0 && c.expires > kMaxCookieExpiresDateInSeconds), "Cookie should have a valid expires, only -1 or a positive number for the unix timestamp in seconds is allowed");
    const copy = { ...c };
    if (copy.url) {
      assert.assert(copy.url !== "about:blank", `Blank page can not have cookie "${c.name}"`);
      assert.assert(!copy.url.startsWith("data:"), `Data URL page can not have cookie "${c.name}"`);
...
class Request extends instrumentation.SdkObject {
  constructor(context, frame, serviceWorker, redirectedFrom, documentId, url, resourceType, method, postData, headers) {
    ...
    this._rawRequestHeadersPromise = new manualPromise.ManualPromise();
    this._waitForResponsePromise = new manualPromise.ManualPromise();
    this._responseEndTiming = -1;
    assert.assert(!url.startsWith("data:"), "Data urls should not fire requests");
...
class Route extends instrumentation.SdkObject {
  ...
  async abort(errorCode = "failed") {
    this._startHandling();
    this._request._context.emit(browserContext.BrowserContext.Events.RequestAborted, this._request);
    await this._delegate.abort(errorCode);
    this._endHandling();
  }
  redirectNavigationRequest(url) {
    this._startHandling();
    assert.assert(this._request.isNavigationRequest());
  ...
  async fulfill(overrides) {
    ...
    if (body === void 0) {
      if (overrides.fetchResponseUid) {
        const buffer = this._request._context.fetchRequest.fetchResponses.get(overrides.fetchResponseUid) || fetch.APIRequestContext.findResponseBody(overrides.fetchResponseUid);
        assert.assert(buffer, "Fetch response has been disposed");
    ...
    this._request._context.emit(browserContext.BrowserContext.Events.RequestFulfilled, this._request);
  ...
  async continue(overrides) {
    ...
    if (!overrides.isFallback)
      this._request._context.emit(browserContext.BrowserContext.Events.RequestContinued, this._request);
    this._startHandling();
    await this._delegate.continue(overrides);
    this._endHandling();
  }
  _startHandling() {
    assert.assert(!this._handled, "Route is already handled!");
    this._handled = true;
...
class Response extends instrumentation.SdkObject {
  constructor(request, status, statusText2, headers, timing, getResponseBodyCallback, fromServiceWorker, httpVersion) {
    ...
    this._headersMap = /* @__PURE__ */ new Map();
    this._serverAddrPromise = new manualPromise.ManualPromise();
    this._securityDetailsPromise = new manualPromise.ManualPromise();
    this._rawResponseHeadersPromise = new manualPromise.ManualPromise();
    this._encodedBodySizePromise = new manualPromise.ManualPromise();
    this._transferSizePromise = new manualPromise.ManualPromise();
    this._responseHeadersSizePromise = new manualPromise.ManualPromise();
    this._request = request;
    this._timing = timing;
    this._status = status;
...
class WebSocket extends instrumentation.SdkObject {
