...
class BidiNetworkManager {
  constructor(bidiSession, page, onNavigationResponseStarted) {
    ...
    this._eventListeners = [
      eventsHelper.eventsHelper.addEventListener(bidiSession, "network.beforeRequestSent", this._onBeforeRequestSent.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "network.responseStarted", this._onResponseStarted.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "network.responseCompleted", this._onResponseCompleted.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "network.fetchError", this._onFetchError.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "network.authRequired", this._onAuthRequired.bind(this))
    ];
  }
  dispose() {
  ...
  async _updateProtocolRequestInterception(initial) {
    ...
    if (enabled) {
      interceptPromise = this._session.send("network.addIntercept", {
        phases: [bidiProtocolCore.Network.InterceptPhase.AuthRequired, bidiProtocolCore.Network.InterceptPhase.BeforeRequestSent],
...
function toBidiResponseHeaders(headers) {
  ...
  const rawCookies = setCookieHeaders.map((h) => cookieStore.parseRawCookie(h.value));
