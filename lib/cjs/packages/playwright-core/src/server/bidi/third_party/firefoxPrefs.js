...
async function createProfile(options) {
  if (!fs.default.existsSync(options.path)) {
    await fs.default.promises.mkdir(options.path, {
      recursive: true
    });
  }
...
async function writePreferences(options) {
  ...
  const result = await Promise.allSettled([
    fs.default.promises.writeFile(path.join(options.path, "user.js"), lines.join("\n")),
    // Create a backup of the preferences file if it already exitsts.
    fs.default.promises.access(prefsPath, fs.default.constants.F_OK).then(
      async () => {
        await fs.default.promises.copyFile(
          prefsPath,
