...
class BidiChromium extends browserType.BrowserType {
  constructor(parent) {
    super(parent, "_bidiChromium");
  }
  async connectToTransport(transport, options, browserLogsCollector) {
    const bidiTransport = await empty.default.connectBidiOverCdp(transport);
    transport[kBidiOverCdpWrapper] = bidiTransport;
    try {
      return bidiBrowser.BidiBrowser.connect(this.attribution.playwright, bidiTransport, options);
    } catch (e) {
  ...
  async waitForReadyState(options, browserLogsCollector) {
    return chromium.waitForReadyState({ ...options, cdpPort: 0 }, browserLogsCollector);
  }
  _innerDefaultArgs(options) {
    ...
    const chromeArguments = [...chromiumSwitches.chromiumSwitches(options.assistantMode)];
