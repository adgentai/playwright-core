...
const bidiExecutionContext = require('./bidiExecutionContext.js');
const bidiInput = require('./bidiInput.js');
const bidiNetworkManager = require('./bidiNetworkManager.js');
const bidiPdf = require('./bidiPdf.js');
...
class BidiPage {
  constructor(browserContext, bidiSession, opener) {
    ...
    this._opener = opener;
    this.rawKeyboard = new bidiInput.RawKeyboardImpl(bidiSession);
    this.rawMouse = new bidiInput.RawMouseImpl(bidiSession);
    this.rawTouchscreen = new bidiInput.RawTouchscreenImpl(bidiSession);
    this._realmToContext = /* @__PURE__ */ new Map();
    this._page = new page.Page(this, browserContext);
    this._browserContext = browserContext;
    this._networkManager = new bidiNetworkManager.BidiNetworkManager(this._session, this._page, this._onNavigationResponseStarted.bind(this));
    this._pdf = new bidiPdf.BidiPDF(this._session);
    this._page.on(page.Page.Events.FrameDetached, (frame) => this._removeContextsForFrame(frame, false));
    this._sessionListeners = [
      eventsHelper.eventsHelper.addEventListener(bidiSession, "script.realmCreated", this._onRealmCreated.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "script.message", this._onScriptMessage.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "browsingContext.contextDestroyed", this._onBrowsingContextDestroyed.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "browsingContext.navigationStarted", this._onNavigationStarted.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "browsingContext.navigationAborted", this._onNavigationAborted.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "browsingContext.navigationFailed", this._onNavigationFailed.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "browsingContext.fragmentNavigated", this._onFragmentNavigated.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "browsingContext.domContentLoaded", this._onDomContentLoaded.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "browsingContext.load", this._onLoad.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "browsingContext.userPromptOpened", this._onUserPromptOpened.bind(this)),
      eventsHelper.eventsHelper.addEventListener(bidiSession, "log.entryAdded", this._onLogEntryAdded.bind(this))
    ];
  ...
  _onRealmCreated(realmInfo) {
    ...
    const delegate = new bidiExecutionContext.BidiExecutionContext(this._session, realmInfo);
  ...
  _onLogEntryAdded(params) {
    ...
    this._page.addConsoleMessage(entry.method, entry.args.map((arg) => bidiExecutionContext.createHandle(context, arg)), location, params.text || void 0);
  }
  ...
  async reload() {
    await this._session.send("browsingContext.reload", {
      context: this._session.sessionId,
      // ignoreCache: true,
      wait: bidiProtocolCore.BrowsingContext.ReadinessState.Interactive
    });
  }
  async goBack() {
...
exports.kPlaywrightBindingChannel = kPlaywrightBindingChannel;
