...
const bidiPage = require('./bidiPage.js');
const page = require('../page.js');
const bidiProtocolCore = require('./third_party/bidiProtocolCore.js');
const bidiProtocolPermissions = require('./third_party/bidiProtocolPermissions.js');

class BidiBrowser extends browser.Browser {
  constructor(parent, transport, options) {
    super(parent, options);
    this._contexts = /* @__PURE__ */ new Map();
    this._bidiPages = /* @__PURE__ */ new Map();
    this._connection = new bidiConnection.BidiConnection(transport, this._onDisconnect.bind(this), options.protocolLogger, options.browserLogsCollector);
    this._browserSession = this._connection.browserSession;
    this._eventListeners = [
      eventsHelper.eventsHelper.addEventListener(this._browserSession, "browsingContext.contextCreated", this._onBrowsingContextCreated.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._browserSession, "script.realmDestroyed", this._onScriptRealmDestroyed.bind(this))
    ];
  }
      ...
      capabilities: {
        alwaysMatch: {
          ...
          unhandledPromptBehavior: {
            default: bidiProtocolCore.Session.UserPromptHandlerType.Ignore
          },
          webSocketUrl: true
        }
      }
  ...
  _onBrowsingContextCreated(event) {
    ...
    const page = new bidiPage.BidiPage(context, session, opener || null);
...
class BidiBrowserContext extends browserContext.BrowserContext {
  ...
  async _createNewPageImpl(markAsServerSideOnly) {
    const { context } = await this._browser._browserSession.send("browsingContext.create", {
      type: bidiProtocolCore.BrowsingContext.CreateType.Window,
  ...
  async doGetCookies(urls) {
    ...
    return network.filterCookies(cookies.map((c) => {
      const copy = {
        name: c.name,
        value: bidiNetworkManager.bidiBytesValueToString(c.value),
        domain: c.domain,
        path: c.path,
        httpOnly: c.httpOnly,
        secure: c.secure,
  ...
  async doGrantPermissions(origin, permissions) {
    ...
    await Promise.all(toGrant.map((permission) => this._setPermission(origin, permission, bidiProtocolPermissions.Permissions.PermissionState.Granted)));
  }
  async doClearPermissions() {
    ...
    await Promise.all(currentPermissions.map(([origin, permissions]) => permissions.map(
      (p) => this._setPermission(origin, p, bidiProtocolPermissions.Permissions.PermissionState.Prompt)
    )));
  }
  ...
  async doExposePlaywrightBinding() {
    const args = [{
      type: "channel",
      value: {
        channel: bidiPage.kPlaywrightBindingChannel,
        ownership: bidiProtocolCore.Script.ResultOwnership.Root
      }
    }];
    const functionDeclaration = `function addMainBinding(callback) { globalThis['${page.PageBinding.kBindingName}'] = callback; }`;
    const promises = [];
...
function toBidiSameSite(sameSite) {
  switch (sameSite) {
    case "Strict":
      return bidiProtocolCore.Network.SameSite.Strict;
    case "Lax":
      return bidiProtocolCore.Network.SameSite.Lax;
    case "None":
      return bidiProtocolCore.Network.SameSite.None;
  }
  return bidiProtocolCore.Network.SameSite.None;
}
