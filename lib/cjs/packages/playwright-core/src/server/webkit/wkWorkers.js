...
class WKWorkers {
  ...
  setSession(session) {
    ...
    this.clear();
    this._sessionListeners = [
      eventsHelper.eventsHelper.addEventListener(session, "Worker.workerCreated", (event) => {
        const worker = new page.Worker(this._page, event.url);
        const workerSession = new wkConnection.WKSession(session.connection, event.workerId, (message) => {
        ...
        worker.createExecutionContext(new wkExecutionContext.WKExecutionContext(workerSession, void 0));
      ...
      eventsHelper.eventsHelper.addEventListener(session, "Worker.dispatchMessageFromWorker", (event) => {
        const workerSession = this._workerSessions.get(event.workerId);
        if (!workerSession)
          return;
        workerSession.dispatchMessage(JSON.parse(event.message));
      }),
      eventsHelper.eventsHelper.addEventListener(session, "Worker.workerTerminated", (event) => {
        const workerSession = this._workerSessions.get(event.workerId);
        if (!workerSession)
          return;
  ...
  async _onConsoleMessage(worker, event) {
    ...
    const handles = (parameters || []).map((p) => {
      return wkExecutionContext.createHandle(worker.existingExecutionContext, p);
    });
    const location = {
      url: url || "",
