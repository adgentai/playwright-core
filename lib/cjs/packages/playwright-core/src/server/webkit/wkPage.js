...
const hostPlatform = require('../utils/hostPlatform.js');
require('node:http');
require('node:http2');
require('node:https');
require('node:url');
...
const wkAccessibility = require('./wkAccessibility.js');
const wkConnection = require('./wkConnection.js');
const wkExecutionContext = require('./wkExecutionContext.js');
const wkInput = require('./wkInput.js');
const wkInterceptableRequest = require('./wkInterceptableRequest.js');
const wkProvisionalPage = require('./wkProvisionalPage.js');
const wkWorkers = require('./wkWorkers.js');
...
class WKPage {
  constructor(browserContext, pageProxySession, opener) {
    ...
    this._opener = opener;
    this.rawKeyboard = new wkInput.RawKeyboardImpl(pageProxySession);
    this.rawMouse = new wkInput.RawMouseImpl(pageProxySession);
    this.rawTouchscreen = new wkInput.RawTouchscreenImpl(pageProxySession);
    this._contextIdToContext = /* @__PURE__ */ new Map();
    this._page = new page.Page(this, browserContext);
    this.rawMouse.setPage(this._page);
    this._workers = new wkWorkers.WKWorkers(this._page);
    this._session = void 0;
    this._browserContext = browserContext;
    this._page.on(page.Page.Events.FrameDetached, (frame) => this._removeContextsForFrame(frame, false));
    this._eventListeners = [
      eventsHelper.eventsHelper.addEventListener(this._pageProxySession, "Target.targetCreated", this._onTargetCreated.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._pageProxySession, "Target.targetDestroyed", this._onTargetDestroyed.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._pageProxySession, "Target.dispatchMessageFromTarget", this._onDispatchMessageFromTarget.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._pageProxySession, "Target.didCommitProvisionalTarget", this._onDidCommitProvisionalTarget.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._pageProxySession, "Screencast.screencastFrame", this._onScreencastFrame.bind(this))
    ];
    ...
    if (opener && !browserContext._options.noDefaultViewport && opener._nextWindowOpenPopupFeatures) {
      const viewportSize = helper.helper.getViewportSizeFromWindowFeatures(opener._nextWindowOpenPopupFeatures);
  ...
  async _initializePageProxySession() {
    ...
    if (this._browserContext._options.recordVideo) {
      const outputFile = path.join(this._browserContext._options.recordVideo.dir, crypto.createGuid() + ".webm");
  ...
  async _initializeSessionMayThrow(session, resourceTreeHandler) {
    ...
    if (this._page.browserContext.needsPlaywrightBinding())
      promises.push(session.send("Runtime.addBinding", { name: page.PageBinding.kBindingName }));
    ...
    promises.push(session.send("Network.setExtraHTTPHeaders", { headers: headers.headersArrayToObject(
  ...
  _onDidCommitProvisionalTarget(event) {
    ...
    assert.assert(this._provisionalPage);
    assert.assert(this._provisionalPage._session.sessionId === newTargetId, "Unknown new target: " + newTargetId);
    assert.assert(this._session.sessionId === oldTargetId, "Unknown old target: " + oldTargetId);
  ...
  didClose() {
    ...
    this._firstNonInitialNavigationCommittedReject(new errors.TargetClosedError());
  ...
  async _onTargetCreated(event) {
    ...
    const session = new wkConnection.WKSession(this._pageProxySession.connection, targetInfo.targetId, (message) => {
    ...
    assert.assert(targetInfo.type === "page", "Only page targets are expected in WebKit, received: " + targetInfo.type);
    if (!targetInfo.isProvisional) {
      assert.assert(!this._page.initializedOrUndefined());
      let pageOrError;
      try {
      ...
      if (pageOrError instanceof page.Page && this._page.mainFrame().url() === "") {
        try {
      ...
      this._page.reportAsNew(this._opener?._page, pageOrError instanceof page.Page ? void 0 : pageOrError);
    } else {
      assert.assert(targetInfo.isProvisional);
      assert.assert(!this._provisionalPage);
      this._provisionalPage = new wkProvisionalPage.WKProvisionalPage(session, this);
      if (targetInfo.isPaused) {
  ...
  _addSessionListeners() {
    this._sessionListeners = [
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.frameNavigated", (event) => this._onFrameNavigated(event.frame, false)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.navigatedWithinDocument", (event) => this._onFrameNavigatedWithinDocument(event.frameId, event.url)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.frameAttached", (event) => this._onFrameAttached(event.frameId, event.parentFrameId)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.frameDetached", (event) => this._onFrameDetached(event.frameId)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.willCheckNavigationPolicy", (event) => this._onWillCheckNavigationPolicy(event.frameId)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.didCheckNavigationPolicy", (event) => this._onDidCheckNavigationPolicy(event.frameId, event.cancel)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.frameScheduledNavigation", (event) => this._onFrameScheduledNavigation(event.frameId, event.delay, event.targetIsCurrentFrame)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.loadEventFired", (event) => this._page.frameManager.frameLifecycleEvent(event.frameId, "load")),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.domContentEventFired", (event) => this._page.frameManager.frameLifecycleEvent(event.frameId, "domcontentloaded")),
      eventsHelper.eventsHelper.addEventListener(this._session, "Runtime.executionContextCreated", (event) => this._onExecutionContextCreated(event.context)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Runtime.bindingCalled", (event) => this._onBindingCalled(event.contextId, event.argument)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Console.messageAdded", (event) => this._onConsoleMessage(event)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Console.messageRepeatCountUpdated", (event) => this._onConsoleRepeatCountUpdated(event)),
      eventsHelper.eventsHelper.addEventListener(this._pageProxySession, "Dialog.javascriptDialogOpening", (event) => this._onDialog(event)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.fileChooserOpened", (event) => this._onFileChooserOpened(event)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Network.requestWillBeSent", (e) => this._onRequestWillBeSent(this._session, e)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Network.requestIntercepted", (e) => this._onRequestIntercepted(this._session, e)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Network.responseReceived", (e) => this._onResponseReceived(this._session, e)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Network.loadingFinished", (e) => this._onLoadingFinished(e)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Network.loadingFailed", (e) => this._onLoadingFailed(this._session, e)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Network.webSocketCreated", (e) => this._page.frameManager.onWebSocketCreated(e.requestId, e.url)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Network.webSocketWillSendHandshakeRequest", (e) => this._page.frameManager.onWebSocketRequest(e.requestId)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Network.webSocketHandshakeResponseReceived", (e) => this._page.frameManager.onWebSocketResponse(e.requestId, e.response.status, e.response.statusText)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Network.webSocketFrameSent", (e) => e.response.payloadData && this._page.frameManager.onWebSocketFrameSent(e.requestId, e.response.opcode, e.response.payloadData)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Network.webSocketFrameReceived", (e) => e.response.payloadData && this._page.frameManager.webSocketFrameReceived(e.requestId, e.response.opcode, e.response.payloadData)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Network.webSocketClosed", (e) => this._page.frameManager.webSocketClosed(e.requestId)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Network.webSocketFrameError", (e) => this._page.frameManager.webSocketError(e.requestId, e.errorMessage))
    ];
  }
  ...
  _onExecutionContextCreated(contextPayload) {
    ...
    if (!frame)
      return;
    const delegate = new wkExecutionContext.WKExecutionContext(this._session, contextPayload.id);
    let worldName = null;
  ...
  async navigateFrame(frame, url, referrer) {
    if (this._pageProxySession.isDisposed())
      throw new errors.TargetClosedError();
  ...
  _onConsoleMessage(event) {
    ...
    if (level === "error" && source === "javascript") {
      const { name, message } = stackTrace.splitErrorMessage(text);
      let stack;
      ...
      error.stack = stack;
      error.name = name;
      this._page.emitOnContextOnceInitialized(browserContext.BrowserContext.Events.PageError, error, this._page);
      return;
    }
    ...
    for (const p of parameters || []) {
      ...
      if (!context)
        return;
      handles.push(wkExecutionContext.createHandle(context, p));
    }
  ...
  async _onFileChooserOpened(event) {
    ...
    try {
      ...
      handle = wkExecutionContext.createHandle(context, event.element).asElement();
    } catch (e) {
      return;
    }
  ...
  async updateExtraHTTPHeaders() {
    await this._updateState("Network.setExtraHTTPHeaders", { headers: headers.headersArrayToObject(
      this._calculateExtraHTTPHeaders(),
      false
      /* lowerCase */
    ) });
  ...
  async exposePlaywrightBinding() {
    await this._updateState("Runtime.addBinding", { name: page.PageBinding.kBindingName });
  }
  ...
  _toolbarHeight() {
    if (this._page.browserContext._browser?.options.headful)
      return hostPlatform.hostPlatform === "mac10.15" ? 55 : 59;
    return 0;
  }
  async _startVideo(options) {
    assert.assert(!this._recordingVideoFile);
  ...
  async takeScreenshot(progress, format, documentRect, viewportRect, quality, fitsViewport, scale) {
    ...
    if (format === "jpeg")
      buffer = utilsBundle.jpegjs.encode(utilsBundle.PNG.sync.read(buffer), quality).data;
    return buffer;
  }
  ...
  _onScreencastFrame(event) {
    ...
    this._page.throttleScreencastFrameAck(() => {
      this._pageProxySession.send("Screencast.screencastFrameAck", { generation }).catch((e) => debugLogger.debugLogger.log("error", e));
    });
    const buffer = Buffer.from(event.data, "base64");
    this._page.emit(page.Page.Events.ScreencastFrame, {
      buffer,
  ...
  async adoptElementHandle(handle, to) {
    ...
    return wkExecutionContext.createHandle(to, result.object);
  }
  async getAccessibilityTree(needle) {
    return wkAccessibility.getAccessibilityTree(this._session, needle);
  }
  ...
  async getFrameElement(frame) {
    ...
    if (!result || result.object.subtype === "null")
      throw new Error("Frame has been detached.");
    return wkExecutionContext.createHandle(context, result.object);
  }
  ...
  _onRequest(session, event, intercepted) {
    ...
    const request = new wkInterceptableRequest.WKInterceptableRequest(session, frame, event, redirectedFrom, documentId);
    let route;
    if (intercepted) {
      route = new wkInterceptableRequest.WKRouteImpl(session, event.requestId);
  ...
  _handleRequestRedirect(request, requestId, responsePayload, timestamp) {
    ...
    response._requestFinished(responsePayload.timing ? helper.helper.secondsToRoundishMillis(timestamp - request._timestamp) : -1);
    this._requestIdToRequest.delete(requestId);
  ...
  _onLoadingFinished(event) {
    ...
    if (response) {
      ...
      response._requestFinished(helper.helper.secondsToRoundishMillis(event.timestamp - request._timestamp));
    } else {
      request.request.setRawRequestHeaders(null);
    }
