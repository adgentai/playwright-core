...
class FrameSelectors {
  ...
  _jumpToAriaRefFrameIfNeeded(selector, info, frame) {
    ...
    if (!jumptToFrame)
      throw new cssParser.InvalidSelectorError(`Invalid frame in aria-ref selector "${selector}"`);
    return jumptToFrame;
  }
    ...
    for (const chunk of frameChunks) {
      selectorParser.visitAllSelectorParts(chunk, (part, nested) => {
        if (nested && part.name === "internal:control" && part.body === "enter-frame") {
          const locator = locatorGenerators.asLocator(this.frame._page.browserContext._browser.sdkLanguage(), selector);
