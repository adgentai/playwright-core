...
const harTracer = require('../../har/harTracer.js');
...
class Tracing extends instrumentation.SdkObject {
  constructor(context, tracesDir) {
    super(context, "tracing");
    this._fs = new fileUtils.SerializedFS();
    ...
    this._harTracer = new harTracer.HarTracer(context, null, this, {
      content: "attach",
    ...
    if (context instanceof browserContext.BrowserContext) {
      this._snapshotter = new snapshotter.Snapshotter(context, this);
      assert.assert(tracesDir, "tracesDir must be specified for BrowserContext");
  ...
  _sdkLanguage() {
    return this._context instanceof browserContext.BrowserContext ? this._context._browser.sdkLanguage() : this._context.attribution.playwright.options.sdkLanguage;
  }
  ...
  start(options) {
    ...
    const traceName = options.name || crypto.createGuid();
  ...
  async startChunk(progress, options = {}) {
    ...
    const preserveNetworkResources = this._context instanceof browserContext.BrowserContext;
    ...
    this._eventListeners.push(
      eventsHelper.eventsHelper.addEventListener(this._context, browserContext.BrowserContext.Events.Console, this._onConsoleMessage.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._context, browserContext.BrowserContext.Events.PageError, this._onPageError.bind(this))
    );
  ...
  _startScreencast() {
    ...
    this._screencastListeners.push(
      eventsHelper.eventsHelper.addEventListener(this._context, browserContext.BrowserContext.Events.Page, this._startScreencastInPage.bind(this))
    );
  }
  _stopScreencast() {
    eventsHelper.eventsHelper.removeEventListeners(this._screencastListeners);
    if (!(this._context instanceof browserContext.BrowserContext))
      return;
    for (const page of this._context.pages())
  ...
  async deleteTmpTracesDir() {
    if (this._tracesTmpDir)
      await fileUtils.removeFolders([this._tracesTmpDir]);
  }
  _createTracesDirIfNeeded() {
    if (this._precreatedTracesDir)
      return this._precreatedTracesDir;
    this._tracesTmpDir = fs.default.mkdtempSync(path.join(os.tmpdir(), "playwright-tracing-"));
  ...
  async stopChunk(progress$1, params) {
    if (this._isStopping)
      throw new Error(`Tracing is already stopping`);
    ...
    const error = await progress$1.race(this._fs.syncAndGetError()).catch((e) => e);
    this._isStopping = false;
    if (this._state)
      this._state.recording = false;
    if (error) {
      if (!progress.isAbortError(error) && this._context instanceof browserContext.BrowserContext && !this._context._browser.isConnected())
        return {};
      throw error;
    }
    if (params.mode === "entries")
      return { entries };
    const artifact$1 = new artifact.Artifact(this._context, zipFileName);
    artifact$1.reportFinished();
  ...
  _onPageError(error, page) {
    const event = {
      ...
      class: "BrowserContext",
      method: "pageError",
      params: { error: errors.serializeError(error) },
      pageId: page.guid
    };
    this._appendTraceEvent(event);
  }
  _startScreencastInPage(page$1) {
    page$1.setScreencastOptions(kScreencastOptions);
    const prefix = page$1.guid;
    this._screencastListeners.push(
      eventsHelper.eventsHelper.addEventListener(page$1, page.Page.Events.ScreencastFrame, (params) => {
...
function visitTraceEvent(object, sha1s) {
  ...
  if (object instanceof dispatcher.Dispatcher)
    return `<${object._type}>`;
