...
class ElementHandleDispatcher extends jsHandleDispatcher.JSHandleDispatcher {
  ...
  static fromJSOrElementHandle(scope, handle) {
    ...
    if (!elementHandle)
      return new jsHandleDispatcher.JSHandleDispatcher(scope, handle);
  ...
  async ownerFrame(params, progress) {
    ...
    return { frame: frame ? frameDispatcher.FrameDispatcher.from(this._browserContextDispatcher(), frame) : void 0 };
  }
  ...
  async dispatchEvent(params, progress) {
    await this._elementHandle.dispatchEvent(progress, params.type, jsHandleDispatcher.parseArgument(params.eventInit));
  }
  ...
  async evalOnSelector(params, progress) {
    return { value: jsHandleDispatcher.serializeResult(await progress.race(this._elementHandle.evalOnSelector(params.selector, !!params.strict, params.expression, params.isFunction, jsHandleDispatcher.parseArgument(params.arg)))) };
  }
  async evalOnSelectorAll(params, progress) {
    return { value: jsHandleDispatcher.serializeResult(await progress.race(this._elementHandle.evalOnSelectorAll(params.selector, params.expression, params.isFunction, jsHandleDispatcher.parseArgument(params.arg)))) };
  }
  ...
  _browserContextDispatcher() {
    ...
    if (parentScope instanceof browserContextDispatcher.BrowserContextDispatcher)
      return parentScope;
