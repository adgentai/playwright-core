...
class RequestDispatcher extends dispatcher.Dispatcher {
  ...
  constructor(scope, request) {
    ...
    const pageDispatcher$1 = page ? scope.connection.existingDispatcher(page) : null;
    const frameDispatcher$1 = frame ? frameDispatcher.FrameDispatcher.from(scope, frame) : null;
    super(pageDispatcher$1 || frameDispatcher$1 || scope, request, "Request", {
      frame: frameDispatcher.FrameDispatcher.fromNullable(scope, request.frame()),
      serviceWorker: pageDispatcher.WorkerDispatcher.fromNullable(scope, request.serviceWorker()),
      url: request.url(),
...
class ResponseDispatcher extends dispatcher.Dispatcher {
...
class RouteDispatcher extends dispatcher.Dispatcher {
...
class WebSocketDispatcher extends dispatcher.Dispatcher {
  constructor(scope, webSocket) {
    ...
    this.addObjectListener(network.WebSocket.Events.FrameSent, (event) => this._dispatchEvent("frameSent", event));
    this.addObjectListener(network.WebSocket.Events.FrameReceived, (event) => this._dispatchEvent("frameReceived", event));
    this.addObjectListener(network.WebSocket.Events.SocketError, (error) => this._dispatchEvent("socketError", { error }));
    this.addObjectListener(network.WebSocket.Events.Close, () => this._dispatchEvent("close", {}));
  }
}
class APIRequestContextDispatcher extends dispatcher.Dispatcher {
  constructor(parentScope, request) {
    const tracing = tracingDispatcher.TracingDispatcher.from(parentScope, request.tracing());
