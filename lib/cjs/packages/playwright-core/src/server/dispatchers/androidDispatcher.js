...
class AndroidDispatcher extends dispatcher.Dispatcher {
...
class AndroidDeviceDispatcher extends dispatcher.Dispatcher {
  constructor(scope, device) {
    ...
    this.addObjectListener(android.AndroidDevice.Events.WebViewAdded, (webView) => this._dispatchEvent("webViewAdded", { webView }));
    this.addObjectListener(android.AndroidDevice.Events.WebViewRemoved, (socketName) => this._dispatchEvent("webViewRemoved", { socketName }));
    this.addObjectListener(android.AndroidDevice.Events.Close, () => this._dispatchEvent("close"));
  }
  ...
  async launchBrowser(params, progress) {
    ...
    return { context: browserContextDispatcher.BrowserContextDispatcher.from(this, context) };
  }
  ...
  async connectToWebView(params, progress) {
    if (this.parentScope()._denyLaunch)
      throw new Error(`Launching more browsers is not allowed.`);
    return { context: browserContextDispatcher.BrowserContextDispatcher.from(this, await this._object.connectToWebView(progress, params.socketName)) };
  }
}
class SocketSdkObject extends instrumentation.SdkObject {
  constructor(parent, socket) {
    super(parent, "socket");
    this._socket = socket;
    this._eventListeners = [
      eventsHelper.eventsHelper.addEventListener(socket, "data", (data) => this.emit("data", data)),
      eventsHelper.eventsHelper.addEventListener(socket, "close", () => {
...
class AndroidSocketDispatcher extends dispatcher.Dispatcher {
