...
class WritableStreamSdkObject extends instrumentation.SdkObject {
...
class WritableStreamDispatcher extends dispatcher.Dispatcher {
  ...
  async close(params, progress) {
    ...
    if (this._object.lastModifiedMs)
      await progress.race(fs.default.promises.utimes(this.path(), new Date(this._object.lastModifiedMs), new Date(this._object.lastModifiedMs)));
  }
  path() {
    if (typeof this._object.streamOrDirectory === "string")
