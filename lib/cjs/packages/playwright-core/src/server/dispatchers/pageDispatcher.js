...
class PageDispatcher extends dispatcher.Dispatcher {
  constructor(parentScope, page$1) {
    const mainFrame = frameDispatcher.FrameDispatcher.from(parentScope, page$1.mainFrame());
    super(parentScope, page$1, "Page", {
      mainFrame,
      viewportSize: page$1.emulatedSize()?.viewport,
      isClosed: page$1.isClosed(),
      opener: PageDispatcher.fromNullable(parentScope, page$1.opener())
    });
    ...
    this._requestInterceptor = (route, request) => {
      const matchesSome = this._interceptionUrlMatchers.some((urlMatch$1) => urlMatch.urlMatches(this._page.browserContext._options.baseURL, request.url(), urlMatch$1));
      if (!matchesSome) {
        route.continue({ isFallback: true }).catch(() => {
        });
        return;
      }
      this._dispatchEvent("route", { route: new networkDispatchers.RouteDispatcher(networkDispatchers.RequestDispatcher.from(this.parentScope(), request), route) });
    };
    this.addObjectListener(page.Page.Events.Close, () => {
      this._dispatchEvent("close");
      this._dispose();
    });
    this.addObjectListener(page.Page.Events.Crash, () => this._dispatchEvent("crash"));
    this.addObjectListener(page.Page.Events.Download, (download) => {
      this._dispatchEvent("download", { url: download.url, suggestedFilename: download.suggestedFilename(), artifact: artifactDispatcher.ArtifactDispatcher.from(parentScope, download.artifact) });
    });
    this.addObjectListener(page.Page.Events.EmulatedSizeChanged, () => this._dispatchEvent("viewportSizeChanged", { viewportSize: page$1.emulatedSize()?.viewport }));
    this.addObjectListener(page.Page.Events.FileChooser, (fileChooser) => this._dispatchEvent("fileChooser", {
      element: elementHandlerDispatcher.ElementHandleDispatcher.from(mainFrame, fileChooser.element()),
      isMultiple: fileChooser.isMultiple()
    }));
    this.addObjectListener(page.Page.Events.FrameAttached, (frame) => this._onFrameAttached(frame));
    this.addObjectListener(page.Page.Events.FrameDetached, (frame) => this._onFrameDetached(frame));
    this.addObjectListener(page.Page.Events.LocatorHandlerTriggered, (uid) => this._dispatchEvent("locatorHandlerTriggered", { uid }));
    this.addObjectListener(page.Page.Events.WebSocket, (webSocket) => this._dispatchEvent("webSocket", { webSocket: new networkDispatchers.WebSocketDispatcher(this, webSocket) }));
    this.addObjectListener(page.Page.Events.Worker, (worker) => this._dispatchEvent("worker", { worker: new WorkerDispatcher(this, worker) }));
    this.addObjectListener(page.Page.Events.Video, (artifact) => this._dispatchEvent("video", { artifact: artifactDispatcher.ArtifactDispatcher.from(parentScope, artifact) }));
    if (page$1.video)
      this._dispatchEvent("video", { artifact: artifactDispatcher.ArtifactDispatcher.from(this.parentScope(), page$1.video) });
    const frames = page$1.frameManager.frames();
  ...
  async reload(params, progress) {
    return { response: networkDispatchers.ResponseDispatcher.fromNullable(this.parentScope(), await this._page.reload(progress, params)) };
  }
  async goBack(params, progress) {
    return { response: networkDispatchers.ResponseDispatcher.fromNullable(this.parentScope(), await this._page.goBack(progress, params)) };
  }
  async goForward(params, progress) {
    return { response: networkDispatchers.ResponseDispatcher.fromNullable(this.parentScope(), await this._page.goForward(progress, params)) };
  }
  ...
  async setWebSocketInterceptionPatterns(params, progress) {
    ...
    if (params.patterns.length && !this._routeWebSocketInitScript)
      this._routeWebSocketInitScript = await webSocketRouteDispatcher.WebSocketRouteDispatcher.install(progress, this.connection, this._page);
  }
  ...
  _onFrameAttached(frame) {
    this._dispatchEvent("frameAttached", { frame: frameDispatcher.FrameDispatcher.from(this.parentScope(), frame) });
  }
  _onFrameDetached(frame) {
    this._dispatchEvent("frameDetached", { frame: frameDispatcher.FrameDispatcher.from(this.parentScope(), frame) });
  }
  _onDispose() {
    ...
    if (this._routeWebSocketInitScript)
      webSocketRouteDispatcher.WebSocketRouteDispatcher.uninstall(this.connection, this._page, this._routeWebSocketInitScript).catch(() => {
      });
...
class WorkerDispatcher extends dispatcher.Dispatcher {
  constructor(scope, worker) {
    super(scope, worker, "Worker", {
      url: worker.url
    });
    this._type_Worker = true;
    this.addObjectListener(page.Worker.Events.Close, () => this._dispatchEvent("close"));
  }
  ...
  async evaluateExpression(params, progress) {
    return { value: jsHandleDispatcher.serializeResult(await progress.race(this._object.evaluateExpression(params.expression, params.isFunction, jsHandleDispatcher.parseArgument(params.arg)))) };
  }
  async evaluateExpressionHandle(params, progress) {
    return { handle: jsHandleDispatcher.JSHandleDispatcher.fromJSHandle(this, await progress.race(this._object.evaluateExpressionHandle(params.expression, params.isFunction, jsHandleDispatcher.parseArgument(params.arg)))) };
  }
}
class BindingCallDispatcher extends dispatcher.Dispatcher {
  constructor(scope, name, needsHandle, source, args) {
    const frameDispatcher$1 = frameDispatcher.FrameDispatcher.from(scope.parentScope(), source.frame);
    super(scope, new instrumentation.SdkObject(scope._object, "bindingCall"), "BindingCall", {
      frame: frameDispatcher$1,
      name,
      args: needsHandle ? void 0 : args.map(jsHandleDispatcher.serializeResult),
      handle: needsHandle ? elementHandlerDispatcher.ElementHandleDispatcher.fromJSOrElementHandle(frameDispatcher$1, args[0]) : void 0
    });
  ...
  async resolve(params, progress) {
    this._resolve(jsHandleDispatcher.parseArgument(params.result));
    this._dispose();
  }
  async reject(params, progress) {
    this._reject(errors.parseError(params.error));
    this._dispose();
  }
}
