...
class JSCoverage {
  ...
  async start(options) {
    assert.assert(!this._enabled, "JSCoverage is already enabled");
    const {
    ...
    this._eventListeners = [
      eventsHelper.eventsHelper.addEventListener(this._client, "Debugger.scriptParsed", this._onScriptParsed.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Runtime.executionContextsCleared", this._onExecutionContextsCleared.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Debugger.paused", this._onDebuggerPaused.bind(this))
    ];
    await Promise.all([
...
class CSSCoverage {
  ...
  async start(options) {
    assert.assert(!this._enabled, "CSSCoverage is already enabled");
    ...
    this._eventListeners = [
      eventsHelper.eventsHelper.addEventListener(this._client, "CSS.styleSheetAdded", this._onStyleSheet.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Runtime.executionContextsCleared", this._onExecutionContextsCleared.bind(this))
    ];
    await Promise.all([
