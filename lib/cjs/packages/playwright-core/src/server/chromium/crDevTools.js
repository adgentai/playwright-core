...
class CRDevTools {
    ...
    session.on("Runtime.bindingCalled", async (event) => {
      ...
      if (parsed.method === "getPreferences") {
        if (this._prefs === void 0) {
          try {
            const json = await fs.default.promises.readFile(this._preferencesPath, "utf8");
  ...
  _save() {
    this._savePromise = this._savePromise.then(async () => {
      await fs.default.promises.writeFile(this._preferencesPath, JSON.stringify(this._prefs)).catch((e) => null);
    });
  }
}
