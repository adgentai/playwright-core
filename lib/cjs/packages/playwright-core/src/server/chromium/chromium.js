...
const chromiumSwitches = require('./chromiumSwitches.js');
...
const crDevTools = require('./crDevTools.js');
const browser = require('../browser.js');

const ARTIFACTS_FOLDER = path.join(os.tmpdir(), "playwright-artifacts-");
class Chromium extends browserType.BrowserType {
  constructor(parent) {
  ...
  async _connectOverCDPInternal(progress, endpointURL, options, onClose) {
    let headersMap;
    if (options.headers)
      headersMap = headers.headersArrayToObject(options.headers, false);
    if (!headersMap)
      headersMap = { "User-Agent": userAgent.getUserAgent() };
    else if (headersMap && !Object.keys(headersMap).some((key) => key.toLowerCase() === "user-agent"))
      headersMap["User-Agent"] = userAgent.getUserAgent();
    const artifactsDir = await progress.race(fs.default.promises.mkdtemp(ARTIFACTS_FOLDER));
    const wsEndpoint = await urlToWSEndpoint(progress, endpointURL, headersMap);
    const chromeTransport = await webSocketTransport.WebSocketTransport.connect(progress, wsEndpoint, { headers: headersMap });
    progress.cleanupWhenAborted(() => chromeTransport.close());
    const cleanedUp = new manualPromise.ManualPromise();
    const doCleanup = async () => {
      await fileUtils.removeFolders([artifactsDir]);
      await onClose?.();
      cleanedUp.resolve();
    };
    const doClose = async () => {
    ...
    const browser$1 = await progress.race(crBrowser.CRBrowser.connect(this.attribution.playwright, chromeTransport, browserOptions));
    browser$1._isCollocatedWithServer = false;
    browser$1.on(browser.Browser.Events.Disconnected, doCleanup);
    return browser$1;
  }
  _createDevTools() {
    const directory = index.registry.findExecutable("chromium").directory;
    return directory ? new crDevTools.CRDevTools(path.join(directory, "devtools-preferences.json")) : void 0;
  }
  async connectToTransport(transport, options, browserLogsCollector) {
    ...
    try {
      return await crBrowser.CRBrowser.connect(this.attribution.playwright, transport, options, devtools);
    } catch (e) {
  ...
  attemptToGracefullyCloseBrowser(transport) {
    const message = { method: "Browser.close", id: crConnection.kBrowserCloseMessageId, params: {} };
  ...
  async _launchWithSeleniumHub(progress, hubUrl, options) {
    ...
    if (process.env.SELENIUM_REMOTE_HEADERS) {
      ...
      if (remoteHeaders)
        headers$1 = remoteHeaders;
    }
    progress.log(`<selenium> connecting to ${hubUrl}`);
    const response = await network.fetchData(progress, {
      url: hubUrl + "session",
      method: "POST",
      headers: {
    ...
    const disconnectFromSelenium = async () => {
      ...
      await network.fetchData(void 0, {
      ...
      processLauncher.gracefullyCloseSet.delete(disconnectFromSelenium);
    };
    processLauncher.gracefullyCloseSet.add(disconnectFromSelenium);
    try {
      ...
      if (capabilities["se:cdp"]) {
        ...
        if (endpointURL.hostname === "localhost" || endpointURL.hostname === "127.0.0.1") {
          ...
          try {
            const sessionResponse = await network.fetchData(progress, {
              url: sessionInfoUrl,
              method: "GET",
              headers: headers$1
            }, seleniumErrorHandler);
  ...
  _innerDefaultArgs(options) {
    ...
    const chromeArguments = [...chromiumSwitches.chromiumSwitches(options.assistantMode, options.channel)];
...
async function waitForReadyState(options, browserLogsCollector) {
  if (options.cdpPort === void 0 && !options.args?.some((a) => a.startsWith("--remote-debugging-port")))
    return {};
  const result = new manualPromise.ManualPromise();
...
async function urlToWSEndpoint(progress, endpointURL, headers) {
  ...
  const json = await network.fetchData(
    progress,
    {
      url: httpURL,
      headers
