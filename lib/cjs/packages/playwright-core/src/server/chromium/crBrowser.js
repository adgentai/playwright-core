...
const crConnection = require('./crConnection.js');
const crPage = require('./crPage.js');
const crProtocolHelper = require('./crProtocolHelper.js');
const crServiceWorker = require('./crServiceWorker.js');

class CRBrowser extends browser.Browser {
  constructor(parent, connection, options) {
    ...
    this._connection.on(crConnection.ConnectionEvents.Disconnected, () => this._didDisconnect());
  ...
  static async connect(parent, transport, options, devtools) {
    options = { ...options };
    const connection = new crConnection.CRConnection(parent, transport, options.protocolLogger, options.browserLogsCollector);
  ...
  _onAttachedToTarget({ targetInfo, sessionId, waitingForDebugger }) {
    ...
    assert.assert(targetInfo.browserContextId, "targetInfo: " + JSON.stringify(targetInfo, null, 2));
    ...
    assert.assert(!this._crPages.has(targetInfo.targetId), "Duplicate target " + targetInfo.targetId);
    assert.assert(!this._backgroundPages.has(targetInfo.targetId), "Duplicate target " + targetInfo.targetId);
    assert.assert(!this._serviceWorkers.has(targetInfo.targetId), "Duplicate target " + targetInfo.targetId);
    if (targetInfo.type === "background_page") {
      const backgroundPage = new crPage.CRPage(session, targetInfo.targetId, context, null, { hasUIWindow: false, isBackgroundPage: true });
      this._backgroundPages.set(targetInfo.targetId, backgroundPage);
      return;
    }
    if (targetInfo.type === "page" || treatOtherAsPage) {
      const opener = targetInfo.openerId ? this._crPages.get(targetInfo.openerId) || null : null;
      const crPage$1 = new crPage.CRPage(session, targetInfo.targetId, context, opener, { hasUIWindow: targetInfo.type === "page", isBackgroundPage: false });
      this._crPages.set(targetInfo.targetId, crPage$1);
      return;
    }
    if (targetInfo.type === "service_worker") {
      const serviceWorker = new crServiceWorker.CRServiceWorker(context, session, targetInfo.url);
  ...
  async startTracing(page, options = {}) {
    assert.assert(!this._tracingRecording, "Cannot start recording trace while already recording trace.");
  ...
  async stopTracing() {
    assert.assert(this._tracingClient, "Tracing was not started.");
    const [event] = await Promise.all([
      new Promise((f) => this._tracingClient.once("Tracing.tracingComplete", f)),
      this._tracingClient.send("Tracing.end")
    ]);
    const tracingPath = path.join(this.options.artifactsDir, crypto.createGuid() + ".crtrace");
    await crProtocolHelper.saveProtocolStream(this._tracingClient, event.stream, tracingPath);
    this._tracingRecording = false;
    const artifact$1 = new artifact.Artifact(this, tracingPath);
...
class CRBrowserContext extends browserContext.BrowserContext {
  static {
    this.CREvents = {
  ...
  async _initialize() {
    assert.assert(!Array.from(this._browser._crPages.values()).some((page) => page._browserContext === this));
  ...
  async newCDPSession(page$1) {
    ...
    if (page$1 instanceof page.Page) {
      targetId = page$1.delegate._targetId;
    } else if (page$1 instanceof frames.Frame) {
      const session = page$1._page.delegate._sessions.get(page$1._id);
      if (!session)
