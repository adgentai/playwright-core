...
const crAccessibility = require('./crAccessibility.js');
const crBrowser = require('./crBrowser.js');
const crCoverage = require('./crCoverage.js');
const crDragDrop = require('./crDragDrop.js');
const crExecutionContext = require('./crExecutionContext.js');
const crInput = require('./crInput.js');
const crNetworkManager = require('./crNetworkManager.js');
const crPdf = require('./crPdf.js');
const crProtocolHelper = require('./crProtocolHelper.js');
const defaultFontFamilies = require('./defaultFontFamilies.js');
...
class CRPage {
  constructor(client, targetId, browserContext$1, opener, bits) {
    ...
    const dragManager = new crDragDrop.DragManager(this);
    this.rawKeyboard = new crInput.RawKeyboardImpl(client, browserContext$1._browser._platform() === "mac", dragManager);
    this.rawMouse = new crInput.RawMouseImpl(this, client, dragManager);
    this.rawTouchscreen = new crInput.RawTouchscreenImpl(client);
    this._pdf = new crPdf.CRPDF(client);
    this._coverage = new crCoverage.CRCoverage(client);
    this._browserContext = browserContext$1;
    this._page = new page.Page(this, browserContext$1);
    this.utilityWorldName = `__playwright_utility_world_${this._page.guid}`;
    this._networkManager = new crNetworkManager.CRNetworkManager(this._page, null);
    this.updateOffline();
    ...
    if (opener && !browserContext$1._options.noDefaultViewport) {
      const features = opener._nextWindowOpenPopupFeatures.shift() || [];
      const viewportSize = helper.helper.getViewportSizeFromWindowFeatures(features);
      if (viewportSize)
        this._page.setEmulatedSizeFromWindowOpen({ viewport: viewportSize, screen: viewportSize });
    }
    const createdEvent = this._isBackgroundPage ? crBrowser.CRBrowserContext.CREvents.BackgroundPage : browserContext.BrowserContext.Events.Page;
  ...
  async _forAllFrameSessions(cb) {
    ...
    await Promise.all(frameSessions.map((frameSession) => {
      ...
      return cb(frameSession).catch((e) => {
        if (protocolError.isSessionClosedError(e))
          return;
        throw e;
      });
    }));
  ...
  async takeScreenshot(progress, format, documentRect, viewportRect, quality, fitsViewport, scale) {
    ...
    if (!documentRect) {
      documentRect = {
        ...
        ...helper.helper.enclosingIntSize({
  ...
  async getAccessibilityTree(needle) {
    return crAccessibility.getAccessibilityTree(this._mainFrameSession._client, needle);
  }
  ...
  async getFrameElement(frame) {
    ...
    const { backendNodeId } = await parentSession._client.send("DOM.getFrameOwner", { frameId: frame._id }).catch((e) => {
      if (e instanceof Error && e.message.includes("Frame with the given id was not found."))
        stackTrace.rewriteErrorMessage(e, "Frame has been detached.");
      throw e;
    });
...
class FrameSession {
  ...
  _addRendererListeners() {
    this._eventListeners.push(...[
      eventsHelper.eventsHelper.addEventListener(this._client, "Log.entryAdded", (event) => this._onLogEntryAdded(event)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Page.fileChooserOpened", (event) => this._onFileChooserOpened(event)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Page.frameAttached", (event) => this._onFrameAttached(event.frameId, event.parentFrameId)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Page.frameDetached", (event) => this._onFrameDetached(event.frameId, event.reason)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Page.frameNavigated", (event) => this._onFrameNavigated(event.frame, false)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Page.frameRequestedNavigation", (event) => this._onFrameRequestedNavigation(event)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Page.javascriptDialogOpening", (event) => this._onDialog(event)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Page.navigatedWithinDocument", (event) => this._onFrameNavigatedWithinDocument(event.frameId, event.url)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Runtime.bindingCalled", (event) => this._onBindingCalled(event)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Runtime.consoleAPICalled", (event) => this._onConsoleAPI(event)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Runtime.exceptionThrown", (exception) => this._handleException(exception.exceptionDetails)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Runtime.executionContextCreated", (event) => this._onExecutionContextCreated(event.context)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Runtime.executionContextDestroyed", (event) => this._onExecutionContextDestroyed(event.executionContextId)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Runtime.executionContextsCleared", (event) => this._onExecutionContextsCleared()),
      eventsHelper.eventsHelper.addEventListener(this._client, "Target.attachedToTarget", (event) => this._onAttachedToTarget(event)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Target.detachedFromTarget", (event) => this._onDetachedFromTarget(event))
    ]);
  }
  _addBrowserListeners() {
    this._eventListeners.push(...[
      eventsHelper.eventsHelper.addEventListener(this._client, "Inspector.targetCrashed", (event) => this._onTargetCrashed()),
      eventsHelper.eventsHelper.addEventListener(this._client, "Page.screencastFrame", (event) => this._onScreencastFrame(event)),
      eventsHelper.eventsHelper.addEventListener(this._client, "Page.windowOpen", (event) => this._onWindowOpen(event))
    ]);
  }
  async _initialize(hasUIWindow) {
    ...
    if (!isSettingStorageState && this._isMainFrame() && this._crPage._browserContext._options.recordVideo && hasUIWindow) {
      const screencastId = crypto.createGuid();
      ...
      this._client.send("Page.getFrameTree").then(({ frameTree }) => {
        ...
        if (isInitialEmptyPage) {
          lifecycleEventsEnabled.catch((e) => {
            ...
            this._eventListeners.push(eventsHelper.eventsHelper.addEventListener(this._client, "Page.lifecycleEvent", (event) => this._onLifecycleEvent(event)));
          });
        } else {
          this._firstNonInitialNavigationCommittedFulfill();
          this._eventListeners.push(eventsHelper.eventsHelper.addEventListener(this._client, "Page.lifecycleEvent", (event) => this._onLifecycleEvent(event)));
  ...
  _onExecutionContextCreated(contextPayload) {
    ...
    if (!frame || this._eventBelongsToStaleFrame(frame._id))
      return;
    const delegate = new crExecutionContext.CRExecutionContext(this._client, contextPayload);
    let worldName = null;
  ...
  _onAttachedToTarget(event) {
    ...
    const worker = new page.Worker(this._page, url);
    ...
    session.once("Runtime.executionContextCreated", async (event2) => {
      worker.createExecutionContext(new crExecutionContext.CRExecutionContext(session, event2.context));
    });
    ...
    session.on("Runtime.consoleAPICalled", (event2) => {
      const args = event2.args.map((o) => crExecutionContext.createHandle(worker.existingExecutionContext, o));
      this._page.addConsoleMessage(event2.type, args, crProtocolHelper.toConsoleMessageLocation(event2.stackTrace));
    });
    session.on("Runtime.exceptionThrown", (exception) => this._page.emitOnContextOnceInitialized(browserContext.BrowserContext.Events.PageError, crProtocolHelper.exceptionToError(exception.exceptionDetails), this._page));
  }
  ...
  async _onConsoleAPI(event) {
    ...
    if (!context)
      return;
    const values = event.args.map((arg) => crExecutionContext.createHandle(context, arg));
    this._page.addConsoleMessage(event.type, values, crProtocolHelper.toConsoleMessageLocation(event.stackTrace));
  }
  ...
  _handleException(exceptionDetails) {
    this._page.emitOnContextOnceInitialized(browserContext.BrowserContext.Events.PageError, crProtocolHelper.exceptionToError(exceptionDetails), this._page);
  }
  ...
  _onLogEntryAdded(event) {
    ...
    if (args)
      args.map((arg) => crProtocolHelper.releaseObject(this._client, arg.objectId));
    if (source !== "worker") {
      const location = {
        url: url || "",
  ...
  async _createVideoRecorder(screencastId, options) {
    assert.assert(!this._screencastId);
    const ffmpegPath = index.registry.findExecutable("ffmpeg").executablePathOrDie(this._page.browserContext._browser.sdkLanguage());
    this._videoRecorder = await videoRecorder.VideoRecorder.launch(this._crPage._page, ffmpegPath, options);
    this._screencastId = screencastId;
  }
  async _startVideoRecording(options) {
    const screencastId = this._screencastId;
    assert.assert(screencastId);
    this._page.once(page.Page.Events.Close, () => this._stopVideoRecording().catch(() => {
    }));
  ...
  async _updateViewport(preserveWindowBoundaries) {
    if (this._crPage._browserContext._browser.isClank())
      return;
    assert.assert(this._isMainFrame());
    const options = this._crPage._browserContext._options;
  ...
  async _setDefaultFontFamilies(session) {
    const fontFamilies = defaultFontFamilies.platformToFontFamilies[this._crPage._browserContext._browser._platform()];
  ...
  async exposePlaywrightBinding() {
    await this._client.send("Runtime.addBinding", { name: page.PageBinding.kBindingName });
  }
  ...
  async _adoptBackendNodeId(backendNodeId, to) {
    ...
    return crExecutionContext.createHandle(to, result.object).asElement();
  }
}
