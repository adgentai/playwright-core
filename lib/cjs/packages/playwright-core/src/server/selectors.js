...
class Selectors {
  constructor(engines, testIdAttributeName) {
    this.guid = `selectors@${crypto.createGuid()}`;
  ...
  parseSelector(selector, strict) {
    const parsed = typeof selector === "string" ? selectorParser.parseSelector(selector) : selector;
    let needsMainWorld = false;
    selectorParser.visitAllSelectorParts(parsed, (part) => {
      const name = part.name;
      const custom = this._engines.get(name);
      if (!custom && !this._builtinEngines.has(name))
        throw new cssParser.InvalidSelectorError(`Unknown engine "${name}" while parsing selector ${selectorParser.stringifySelector(parsed)}`);
