...
const injectedScriptSource = require('../generated/injectedScriptSource.js');
...
class FrameExecutionContext extends javascript.ExecutionContext {
  ...
  async evaluate(pageFunction, arg) {
    return javascript.evaluate(this, true, pageFunction, arg);
  }
  async evaluateHandle(pageFunction, arg) {
    return javascript.evaluate(this, false, pageFunction, arg);
  }
  async evaluateExpression(expression, options, arg) {
    return javascript.evaluateExpression(this, expression, { ...options, returnByValue: true }, arg);
  }
  async evaluateExpressionHandle(expression, options, arg) {
    return javascript.evaluateExpression(this, expression, { ...options, returnByValue: false }, arg);
  }
  injectedScript() {
...
class ElementHandle extends javascript.JSHandle {
  ...
  async setInputFiles(progress, params) {
    const inputFileItems = await progress.race(fileUploadUtils.prepareFilesForUpload(this._frame, params));
    await this._markAsTargetElement(progress);
  ...
  async _setInputFiles(progress, items) {
    ...
    if (localPaths || localDirectory) {
      ...
      await progress.race(Promise.all(localPathsOrDirectory.map((localPath) => fs.default.promises.access(localPath, fs.default.constants.F_OK))));
