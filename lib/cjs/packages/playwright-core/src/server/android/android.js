...
class AndroidDevice extends instrumentation.SdkObject {
  ...
  async _installDriver(progress) {
    ...
    if (!this._options.omitDriverInstall) {
      ...
      for (const file of ["android-driver.apk", "android-driver-target.apk"]) {
        ...
        await this.installApk(progress, await progress.race(fs.default.promises.readFile(fullName)));
      }
    } else {
  ...
  async _waitForLocalAbstract(progress$1, socketName) {
    let socket;
    utilsBundle.debug("pw:android")(`Polling the socket localabstract:${socketName}`);
    while (!socket) {
      try {
        socket = await progress$1.raceWithCleanup(this._backend.open(`localabstract:${socketName}`), (socket2) => socket2.close());
      } catch (e) {
        if (progress.isAbortError(e))
          throw e;
        await progress$1.wait(250);
      }
    }
  ...
  async _connectToBrowser(progress, socketName, options = {}) {
    ...
    const browser = await progress.race(crBrowser.CRBrowser.connect(this.attribution.playwright, androidBrowser, browserOptions));
