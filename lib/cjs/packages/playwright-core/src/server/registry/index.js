...
const browsers = require('../../../browsers.json.js');
...
const PACKAGE_PATH = path.join("/home/runner/work/playwright/playwright/packages/playwright-cloudflare", "..", "..", "..");
const BIN_PATH = path.join("/home/runner/work/playwright/playwright/packages/playwright-cloudflare", "..", "..", "..", "bin");
...
const registryDirectory = (() => {
  let result;
  const envDefined = env.getFromENV("PLAYWRIGHT_BROWSERS_PATH");
  if (envDefined === "0") {
    result = path.join("/home/runner/work/playwright/playwright/packages/playwright-cloudflare", "..", "..", "..", ".local-browsers");
  } else if (envDefined) {
    result = envDefined;
  } else {
    let cacheDirectory;
  ...
  if (!path.isAbsolute(result)) {
    result = path.resolve(env.getFromENV("INIT_CWD") || process.cwd(), result);
  }
  return result;
})();
...
function readDescriptors(browsersJSON) {
  return browsersJSON["browsers"].map((obj) => {
    const name = obj.name;
    const revisionOverride = (obj.revisionOverrides || {})[hostPlatform.hostPlatform];
    const revision = revisionOverride || obj.revision;
    const browserDirectoryPrefix = revisionOverride ? `${name}_${hostPlatform.hostPlatform}_special` : `${name}`;
    const descriptor = {
      name,
      revision,
...
class Registry {
  constructor(browsersJSON) {
    ...
    const findExecutablePath = (dir, name) => {
      ...
      return tokens ? path.join(dir, ...tokens) : void 0;
    };
    const executablePathOrDie = (name, e, installByDefault, sdkLanguage) => {
      if (!e)
        throw new Error(`${name} is not supported on ${hostPlatform.hostPlatform}`);
      const installCommand = buildPlaywrightCLICommand(sdkLanguage, `install${installByDefault ? "" : " " + name}`);
      if (!fileUtils.canAccessFile(e)) {
        const currentDockerVersion = dependencies.readDockerVersionSync();
        const preferredDockerVersion = currentDockerVersion ? dependencies.dockerVersion(currentDockerVersion.dockerImageNameTemplate) : null;
...
${ascii.wrapInASCIIBox(prettyMessage, 1)}`);
      }
      return e;
    };
    this._executables = [];
  ...
  _createChromiumChannel(name, lookAt, install) {
    const executablePath = (sdkLanguage, shouldThrow) => {
      ...
      for (const prefix of prefixes) {
        const executablePath2 = path.join(prefix, suffix);
        if (fileUtils.canAccessFile(executablePath2))
...
Run "${buildPlaywrightCLICommand(sdkLanguage, "install " + name)}"` : "";
  ...
  async _validateHostRequirements(sdkLanguage, browserDirectory, linuxLddDirectories, dlOpenLibraries, windowsExeAndDllDirectories) {
    if (os.platform() === "linux")
      return await dependencies.validateDependenciesLinux(sdkLanguage, linuxLddDirectories.map((d) => path.join(browserDirectory, d)), dlOpenLibraries);
    if (os.platform() === "win32" && os.arch() === "x64")
      return await dependencies.validateDependenciesWindows(sdkLanguage, windowsExeAndDllDirectories.map((d) => path.join(browserDirectory, d)));
  }
  async installDeps(executablesToInstallDeps, dryRun) {
    ...
    if (os.platform() === "win32")
      return await dependencies.installDependenciesWindows(targets, dryRun);
    if (os.platform() === "linux")
      return await dependencies.installDependenciesLinux(targets, dryRun);
  }
  async install(executablesToInstall, forceReinstall) {
    const executables = this._dedupe(executablesToInstall);
    await fs.default.promises.mkdir(registryDirectory, { recursive: true });
    const lockfilePath = path.join(registryDirectory, "__dirlock");
    const linksDir = path.join(registryDirectory, ".links");
    let releaseLock;
    try {
      releaseLock = await utilsBundle.lockfile.lock(registryDirectory, {
        retries: {
      ...
      await fs.default.promises.mkdir(linksDir, { recursive: true });
      await fs.default.promises.writeFile(path.join(linksDir, crypto.calculateSha1(PACKAGE_PATH)), PACKAGE_PATH);
      if (!env.getAsBooleanFromENV("PLAYWRIGHT_SKIP_BROWSER_GC"))
        await this._validateInstallationCache(linksDir);
      for (const executable of executables) {
        if (!executable._install)
          throw new Error(`ERROR: Playwright does not support installing ${executable.name}`);
        const { embedderName } = userAgent.getEmbedderName();
        if (!env.getAsBooleanFromENV("CI") && !executable._isHermeticInstallation && !forceReinstall && executable.executablePath(embedderName)) {
  ...
  async uninstall(all) {
    const linksDir = path.join(registryDirectory, ".links");
    if (all) {
      const links = await fs.default.promises.readdir(linksDir).catch(() => []);
      for (const link of links)
        await fs.default.promises.unlink(path.join(linksDir, link));
    } else {
      await fs.default.promises.unlink(path.join(linksDir, crypto.calculateSha1(PACKAGE_PATH))).catch(() => {
      });
    }
    await this._validateInstallationCache(linksDir);
    return {
      numberOfBrowsersLeft: (await fs.default.promises.readdir(registryDirectory).catch(() => [])).filter((browserDirectory) => isBrowserDirectory(browserDirectory)).length
    };
  }
  async validateHostRequirementsForExecutablesIfNeeded(executables, sdkLanguage) {
    if (env.getAsBooleanFromENV("PLAYWRIGHT_SKIP_VALIDATE_HOST_REQUIREMENTS")) {
  ...
  async _validateHostRequirementsForExecutableIfNeeded(executable, sdkLanguage) {
    ...
    if (await fs.default.promises.stat(markerFile).then((stat) => Date.now() - stat.mtime.getTime() < kMaximumReValidationPeriod).catch(() => false))
      return;
    debugLogger.debugLogger.log("install", `validating host requirements for "${executable.name}"`);
    try {
      await executable._validateHostRequirements(sdkLanguage);
      debugLogger.debugLogger.log("install", `validation passed for ${executable.name}`);
    } catch (error) {
      debugLogger.debugLogger.log("install", `validation failed for ${executable.name}`);
      throw error;
    }
    await fs.default.promises.writeFile(markerFile, "").catch(() => {
    });
  }
  _downloadURLs(descriptor) {
    const paths = DOWNLOAD_PATHS[descriptor.name];
    const downloadPathTemplate = paths[hostPlatform.hostPlatform] || paths["<unknown>"];
    if (!downloadPathTemplate)
      return [];
    const downloadPath = require$$0$2__namespace.format(downloadPathTemplate, descriptor.revision);
    ...
    const customHostOverride = downloadHostEnv && env.getFromENV(downloadHostEnv) || env.getFromENV("PLAYWRIGHT_DOWNLOAD_HOST");
  ...
  async _downloadExecutable(descriptor, executablePath) {
    ...
    if (!downloadURLs.length)
      throw new Error(`ERROR: Playwright does not support ${descriptor.name} on ${hostPlatform.hostPlatform}`);
    if (!hostPlatform.isOfficiallySupportedPlatform)
      browserFetcher.logPolitely(`BEWARE: your OS is not officially supported by Playwright; downloading fallback build for ${hostPlatform.hostPlatform}.`);
    if (descriptor.hasRevisionOverride) {
      const message = `You are using a frozen ${descriptor.name} browser which does not receive updates anymore on ${hostPlatform.hostPlatform}. Please update to the latest version of your operating system to test up-to-date browsers.`;
      if (process.env.GITHUB_ACTIONS)
        console.log(`::warning title=Playwright::${message}`);
      else
        browserFetcher.logPolitely(message);
    }
    ...
    const downloadFileName = `playwright-download-${descriptor.name}-${hostPlatform.hostPlatform}-${descriptor.revision}.zip`;
    const downloadSocketTimeoutEnv = env.getFromENV("PLAYWRIGHT_DOWNLOAD_CONNECTION_TIMEOUT");
    const downloadSocketTimeout = +(downloadSocketTimeoutEnv || "0") || network.NET_DEFAULT_TIMEOUT;
    await browserFetcher.downloadBrowserWithProgressBar(title, descriptor.dir, executablePath, downloadURLs, downloadFileName, downloadSocketTimeout).catch((e) => {
...
${e.stack}`);
  ...
  async _installMSEdgeChannel(channel, scripts) {
    const scriptArgs = [];
    if (process.platform !== "linux") {
      const products = lowercaseAllKeys(JSON.parse(await network.fetchData(void 0, { url: "https://edgeupdates.microsoft.com/api/products" })));
      const productName = {
        "msedge": "Stable",
        "msedge-beta": "Beta",
        "msedge-dev": "Dev"
  ...
  async _installChromiumChannel(channel, scripts, scriptArgs = []) {
    ...
    if (isPowerShell) {
      ...
      const { code } = await spawnAsync.spawnAsync("powershell.exe", args, { cwd, stdio: "inherit" });
      if (code !== 0)
        throw new Error(`Failed to install ${channel}`);
    } else {
      const { command, args, elevatedPermissions } = await dependencies.transformCommandsForRoot([`bash "${path.join(BIN_PATH, scriptName)}" ${scriptArgs.join("")}`]);
      if (elevatedPermissions)
        console.log("Switching to root user to install dependencies...");
      const { code } = await spawnAsync.spawnAsync(command, args, { cwd, stdio: "inherit" });
      if (code !== 0)
        throw new Error(`Failed to install ${channel}`);
    }
  }
  async listInstalledBrowsers() {
    ...
    return browsers.filter((browser) => fs.default.existsSync(browser.browserPath));
  }
  ...
  async _traverseBrowserInstallations(linksDir) {
    const browserList = [];
    const brokenLinks = [];
    for (const fileName of await fs.default.promises.readdir(linksDir)) {
      const linkPath = path.join(linksDir, fileName);
      let linkTarget = "";
      try {
        linkTarget = (await fs.default.promises.readFile(linkPath)).toString();
        const browsersJSON = _commonjsDynamicModules.commonjsRequire(path.join(linkTarget, "browsers.json"));
        const descriptors = readDescriptors(browsersJSON);
        for (const browserName of allDownloadable) {
  ...
  async _deleteStaleBrowsers(browserList) {
    ...
    for (const browser of browserList) {
      ...
      if (!shouldHaveMarkerFile || await fileUtils.existsAsync(browserDirectoryToMarkerFilePath(browserPath)))
        usedBrowserPaths.add(browserPath);
    }
    let downloadedBrowsers = (await fs.default.promises.readdir(registryDirectory)).map((file) => path.join(registryDirectory, file));
    ...
    await fileUtils.removeFolders([...directories]);
  }
  async _deleteBrokenInstallations(brokenLinks) {
    for (const linkPath of brokenLinks)
      await fs.default.promises.unlink(linkPath).catch((e) => {
      });
  }
}
...
const registry = new Registry(browsers.default);
