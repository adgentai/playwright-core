...
const ffPage = require('./ffPage.js');
const page = require('../page.js');

class FFBrowser extends browser.Browser {
  constructor(parent, connection, options) {
    ...
    this._connection.on(ffConnection.ConnectionEvents.Disconnected, () => this._onDisconnect());
  ...
  static async connect(parent, transport, options) {
    const connection = new ffConnection.FFConnection(transport, options.protocolLogger, options.browserLogsCollector);
  ...
  _onAttachedToTarget(payload) {
    ...
    assert.assert(type === "page");
    const context = browserContextId ? this._contexts.get(browserContextId) : this._defaultContext;
    assert.assert(context, `Unknown context id:${browserContextId}, _defaultContext: ${this._defaultContext}`);
    const session = this._connection.createSession(payload.sessionId);
    const opener = openerId ? this._ffPages.get(openerId) : null;
    const ffPage$1 = new ffPage.FFPage(session, context, opener);
    this._ffPages.set(targetId, ffPage$1);
  }
...
class FFBrowserContext extends browserContext.BrowserContext {
  constructor(browser, browserContextId, options) {
    super(browser, options, browserContextId);
  }
  async _initialize() {
    assert.assert(!this._ffPages().length);
  ...
  async doExposePlaywrightBinding() {
    this._browser.session.send("Browser.addBinding", { browserContextId: this._browserContextId, name: page.PageBinding.kBindingName, script: "" });
  }
  onClosePersistent() {
  }
  async clearCache() {
