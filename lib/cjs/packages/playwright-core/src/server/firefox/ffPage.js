...
const ffAccessibility = require('./ffAccessibility.js');
const ffConnection = require('./ffConnection.js');
const ffExecutionContext = require('./ffExecutionContext.js');
const ffInput = require('./ffInput.js');
const ffNetworkManager = require('./ffNetworkManager.js');
...
class FFPage {
  constructor(session, browserContext, opener) {
    ...
    this._opener = opener;
    this.rawKeyboard = new ffInput.RawKeyboardImpl(session);
    this.rawMouse = new ffInput.RawMouseImpl(session);
    this.rawTouchscreen = new ffInput.RawTouchscreenImpl(session);
    this._contextIdToContext = /* @__PURE__ */ new Map();
    this._browserContext = browserContext;
    this._page = new page.Page(this, browserContext);
    this.rawMouse.setPage(this._page);
    this._networkManager = new ffNetworkManager.FFNetworkManager(session, this._page);
    this._page.on(page.Page.Events.FrameDetached, (frame) => this._removeContextsForFrame(frame));
    this._eventListeners = [
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.eventFired", this._onEventFired.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.frameAttached", this._onFrameAttached.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.frameDetached", this._onFrameDetached.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.navigationAborted", this._onNavigationAborted.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.navigationCommitted", this._onNavigationCommitted.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.navigationStarted", this._onNavigationStarted.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.sameDocumentNavigation", this._onSameDocumentNavigation.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Runtime.executionContextCreated", this._onExecutionContextCreated.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Runtime.executionContextDestroyed", this._onExecutionContextDestroyed.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Runtime.executionContextsCleared", this._onExecutionContextsCleared.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.linkClicked", (event) => this._onLinkClicked(event.phase)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.uncaughtError", this._onUncaughtError.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Runtime.console", this._onConsole.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.dialogOpened", this._onDialogOpened.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.bindingCalled", this._onBindingCalled.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.fileChooserOpened", this._onFileChooserOpened.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.workerCreated", this._onWorkerCreated.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.workerDestroyed", this._onWorkerDestroyed.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.dispatchMessageFromWorker", this._onDispatchMessageFromWorker.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.crashed", this._onCrashed.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.videoRecordingStarted", this._onVideoRecordingStarted.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.webSocketCreated", this._onWebSocketCreated.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.webSocketClosed", this._onWebSocketClosed.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.webSocketFrameReceived", this._onWebSocketFrameReceived.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.webSocketFrameSent", this._onWebSocketFrameSent.bind(this)),
      eventsHelper.eventsHelper.addEventListener(this._session, "Page.screencastFrame", this._onScreencastFrame.bind(this))
    ];
    ...
    this.addInitScript(new page.InitScript(""), UTILITY_WORLD_NAME).catch((e) => this._markAsError(e));
  }
  ...
  _onExecutionContextCreated(payload) {
    ...
    if (!frame)
      return;
    const delegate = new ffExecutionContext.FFExecutionContext(this._session, executionContextId);
    let worldName = null;
  ...
  _onUncaughtError(params) {
    const { name, message } = stackTrace.splitErrorMessage(params.message);
  ...
  _onConsole(payload) {
    ...
    if (!context)
      return;
    this._page.addConsoleMessage(type === "warn" ? "warning" : type, args.map((arg) => ffExecutionContext.createHandle(context, arg)), location);
  }
  ...
  async _onFileChooserOpened(payload) {
    ...
    if (!context)
      return;
    const handle = ffExecutionContext.createHandle(context, element).asElement();
    await this._page._onFileChooserOpened(handle);
  }
  async _onWorkerCreated(event) {
    const workerId = event.workerId;
    const worker = new page.Worker(this._page, event.url);
    const workerSession = new ffConnection.FFSession(this._session._connection, workerId, (message) => {
    ...
    workerSession.once("Runtime.executionContextCreated", (event2) => {
      worker.createExecutionContext(new ffExecutionContext.FFExecutionContext(workerSession, event2.executionContextId));
    });
    workerSession.on("Runtime.console", (event2) => {
      const { type, args, location } = event2;
      const context = worker.existingExecutionContext;
      this._page.addConsoleMessage(type, args.map((arg) => ffExecutionContext.createHandle(context, arg)), location);
    });
  }
  ...
  didClose() {
    this._markAsError(new errors.TargetClosedError());
  ...
  _onScreencastFrame(event) {
    ...
    this._page.throttleScreencastFrameAck(() => {
      this._session.send("Page.screencastFrameAck", { screencastId }).catch((e) => debugLogger.debugLogger.log("error", e));
    });
  ...
  async adoptElementHandle(handle, to) {
    ...
    return ffExecutionContext.createHandle(to, result.remoteObject);
  }
  async getAccessibilityTree(needle) {
    return ffAccessibility.getAccessibilityTree(this._session, needle);
  }
  ...
  async getFrameElement(frame) {
    ...
    if (!result.remoteObject)
      throw new Error("Frame has been detached.");
    return ffExecutionContext.createHandle(context, result.remoteObject);
  }
