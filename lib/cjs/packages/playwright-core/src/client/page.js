...
class Page extends channelOwner.ChannelOwner {
  constructor(parent, type, guid, initializer) {
    super(parent, type, guid, initializer);
    this._frames = /* @__PURE__ */ new Set();
    this._workers = /* @__PURE__ */ new Set();
    this._closed = false;
    this._closedOrCrashedScope = new manualPromise.LongStandingScope();
    this._routes = [];
    ...
    this._timeoutSettings = new timeoutSettings.TimeoutSettings(this._platform, this._browserContext._timeoutSettings);
    this.accessibility = new accessibility.Accessibility(this._channel);
    this.keyboard = new input.Keyboard(this);
    this.mouse = new input.Mouse(this);
    this.request = this._browserContext.request;
    this.touchscreen = new input.Touchscreen(this);
    this.clock = this._browserContext.clock;
    this._mainFrame = frame.Frame.from(initializer.mainFrame);
    ...
    this._channel.on("download", ({ url, suggestedFilename, artifact: artifact$1 }) => {
      const artifactObject = artifact.Artifact.from(artifact$1);
      this.emit(events.Events.Page.Download, new download.Download(this, url, suggestedFilename, artifactObject));
    });
    this._channel.on("fileChooser", ({ element, isMultiple }) => this.emit(events.Events.Page.FileChooser, new fileChooser.FileChooser(this, elementHandle.ElementHandle.from(element), isMultiple)));
    this._channel.on("frameAttached", ({ frame: frame$1 }) => this._onFrameAttached(frame.Frame.from(frame$1)));
    this._channel.on("frameDetached", ({ frame: frame$1 }) => this._onFrameDetached(frame.Frame.from(frame$1)));
    this._channel.on("locatorHandlerTriggered", ({ uid }) => this._onLocatorHandlerTriggered(uid));
    this._channel.on("route", ({ route }) => this._onRoute(network.Route.from(route)));
    this._channel.on("webSocketRoute", ({ webSocketRoute }) => this._onWebSocketRoute(network.WebSocketRoute.from(webSocketRoute)));
    this._channel.on("video", ({ artifact: artifact$1 }) => {
      const artifactObject = artifact.Artifact.from(artifact$1);
      this._forceVideo()._artifactReady(artifactObject);
    });
    this._channel.on("viewportSizeChanged", ({ viewportSize }) => this._viewportSize = viewportSize);
    this._channel.on("webSocket", ({ webSocket }) => this.emit(events.Events.Page.WebSocket, network.WebSocket.from(webSocket)));
    this._channel.on("worker", ({ worker: worker$1 }) => this._onWorker(worker.Worker.from(worker$1)));
    this.coverage = new coverage.Coverage(this._channel);
    this.once(events.Events.Page.Close, () => this._closedOrCrashedScope.close(this._closeErrorWithReason()));
    this.once(events.Events.Page.Crash, () => this._closedOrCrashedScope.close(new errors.TargetClosedError()));
    this._setEventToSubscriptionMapping(/* @__PURE__ */ new Map([
      [events.Events.Page.Console, "console"],
      [events.Events.Page.Dialog, "dialog"],
      [events.Events.Page.Request, "request"],
      [events.Events.Page.Response, "response"],
      [events.Events.Page.RequestFinished, "requestFinished"],
      [events.Events.Page.RequestFailed, "requestFailed"],
      [events.Events.Page.FileChooser, "fileChooser"]
    ]));
  }
  static from(page) {
    return page._object;
  ...
  _onFrameAttached(frame) {
    ...
    this.emit(events.Events.Page.FrameAttached, frame);
  }
  _onFrameDetached(frame) {
    ...
    this.emit(events.Events.Page.FrameDetached, frame);
  }
  ...
  _onWorker(worker) {
    ...
    worker._page = this;
    this.emit(events.Events.Page.Worker, worker);
  }
  _onClose() {
    this._closed = true;
    ...
    this.emit(events.Events.Page.Close, this);
  }
  _onCrash() {
    this.emit(events.Events.Page.Crash, this);
  }
  context() {
  ...
  frame(frameSelector) {
    const name = stringUtils.isString(frameSelector) ? frameSelector : frameSelector.name;
    const url = rtti.isObject(frameSelector) ? frameSelector.url : void 0;
    assert.assert(name || url, "Either name or url matcher should be specified");
    return this.frames().find((f) => {
      if (name)
        return f.name() === name;
      return urlMatch.urlMatches(this._browserContext._options.baseURL, f.url(), url);
    }) || null;
  }
  frames() {
    return [...this._frames];
  ...
  _forceVideo() {
    if (!this._video)
      this._video = new video.Video(this, this._connection);
    return this._video;
  }
  video() {
  ...
  async $eval(selector, pageFunction, arg) {
    jsHandle.assertMaxArguments(arguments.length, 3);
  ...
  async reload(options = {}) {
    const waitUntil = frame.verifyLoadState("waitUntil", options.waitUntil === void 0 ? "load" : options.waitUntil);
    return network.Response.fromNullable((await this._channel.reload({ ...options, waitUntil, timeout: this._timeoutSettings.navigationTimeout(options) })).response);
  }
  ...
  async waitForRequest(urlOrPredicate, options = {}) {
    const predicate = async (request) => {
      if (stringUtils.isString(urlOrPredicate) || rtti.isRegExp(urlOrPredicate))
        return urlMatch.urlMatches(this._browserContext._options.baseURL, request.url(), urlOrPredicate);
      return await urlOrPredicate(request);
    };
    const trimmedUrl = trimUrl(urlOrPredicate);
    const logLine = trimmedUrl ? `waiting for request ${trimmedUrl}` : void 0;
    return await this._waitForEvent(events.Events.Page.Request, { predicate, timeout: options.timeout }, logLine);
  }
  async waitForResponse(urlOrPredicate, options = {}) {
    const predicate = async (response) => {
      if (stringUtils.isString(urlOrPredicate) || rtti.isRegExp(urlOrPredicate))
        return urlMatch.urlMatches(this._browserContext._options.baseURL, response.url(), urlOrPredicate);
      return await urlOrPredicate(response);
    };
    const trimmedUrl = trimUrl(urlOrPredicate);
    const logLine = trimmedUrl ? `waiting for response ${trimmedUrl}` : void 0;
    return await this._waitForEvent(events.Events.Page.Response, { predicate, timeout: options.timeout }, logLine);
  }
  async waitForEvent(event, optionsOrPredicate = {}) {
    return await this._waitForEvent(event, optionsOrPredicate, `waiting for event "${event}"`);
  }
  _closeErrorWithReason() {
    return new errors.TargetClosedError(this._closeReason || this._browserContext._effectiveCloseReason());
  }
  async _waitForEvent(event, optionsOrPredicate, logLine) {
    return await this._wrapApiCall(async () => {
      ...
      if (logLine)
        waiter$1.log(logLine);
      waiter$1.rejectOnTimeout(timeout, `Timeout ${timeout}ms exceeded while waiting for event "${event}"`);
      if (event !== events.Events.Page.Crash)
        waiter$1.rejectOnEvent(this, events.Events.Page.Crash, new Error("Page crashed"));
      if (event !== events.Events.Page.Close)
        waiter$1.rejectOnEvent(this, events.Events.Page.Close, () => this._closeErrorWithReason());
  ...
  async goBack(options = {}) {
    const waitUntil = frame.verifyLoadState("waitUntil", options.waitUntil === void 0 ? "load" : options.waitUntil);
    return network.Response.fromNullable((await this._channel.goBack({ ...options, waitUntil, timeout: this._timeoutSettings.navigationTimeout(options) })).response);
  }
  async goForward(options = {}) {
    const waitUntil = frame.verifyLoadState("waitUntil", options.waitUntil === void 0 ? "load" : options.waitUntil);
    return network.Response.fromNullable((await this._channel.goForward({ ...options, waitUntil, timeout: this._timeoutSettings.navigationTimeout(options) })).response);
  }
  async requestGC() {
  ...
  async route(url, handler, options = {}) {
    this._routes.unshift(new network.RouteHandler(this._platform, this._browserContext._options.baseURL, url, handler, options.times));
  ...
  async routeFromHAR(har, options = {}) {
    ...
    await harRouter$1.addPageRoute(this);
  }
  async routeWebSocket(url, handler) {
    this._webSocketRoutes.unshift(new network.WebSocketRouteHandler(this._browserContext._options.baseURL, url, handler));
  ...
  async screenshot(options = {}) {
    ...
    if (!copy.type)
      copy.type = elementHandle.determineScreenshotType(options);
    if (mask) {
  ...
  async close(options = {}) {
    ...
    try {
      ...
      if (errors.isTargetClosedError(e) && !options.runBeforeUnload)
        return;
      throw e;
    }
  }
...
class BindingCall extends channelOwner.ChannelOwner {
  static from(channel) {
  ...
  async call(func) {
    try {
      const frame$1 = frame.Frame.from(this._initializer.frame);
      const source = {
        context: frame$1._page.context(),
        page: frame$1._page,
        frame: frame$1
      };
      let result;
      if (this._initializer.handle)
        result = await func(source, jsHandle.JSHandle.from(this._initializer.handle));
      else
        result = await func(source, ...this._initializer.args.map(jsHandle.parseResult));
      this._channel.resolve({ result: jsHandle.serializeArgument(result) }).catch(() => {
      });
    } catch (e) {
      this._channel.reject({ error: errors.serializeError(e) }).catch(() => {
      });
    }
  }
}
function trimUrl(param) {
  if (rtti.isRegExp(param))
    return `/${stringUtils.trimStringWithEllipsis(param.source, 50)}/${param.flags}`;
  if (stringUtils.isString(param))
    return `"${stringUtils.trimStringWithEllipsis(param, 50)}"`;
}
