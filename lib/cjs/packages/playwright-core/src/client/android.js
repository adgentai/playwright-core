...
class Android extends channelOwner.ChannelOwner {
  static from(android) {
  ...
  async connect(wsEndpoint, options = {}) {
    return await this._wrapApiCall(async () => {
      const deadline = options.timeout ? time.monotonicTime() + options.timeout : 0;
      ...
      const result = await timeoutRunner.raceAgainstDeadline(async () => {
        ...
        device.on(events.Events.AndroidDevice.Close, () => connection.close());
        return device;
      }, deadline);
      if (!result.timedOut) {
...
class AndroidDevice extends channelOwner.ChannelOwner {
  constructor(parent, type, guid, initializer) {
    super(parent, type, guid, initializer);
    ...
    this._timeoutSettings = new timeoutSettings.TimeoutSettings(this._platform, parent._timeoutSettings);
  ...
  _onWebViewAdded(webView) {
    ...
    this.emit(events.Events.AndroidDevice.WebView, view);
  }
  _onWebViewRemoved(socketName) {
    const view = this._webViews.get(socketName);
    this._webViews.delete(socketName);
    if (view)
      view.emit(events.Events.AndroidWebView.Close);
  }
  setDefaultTimeout(timeout) {
    this._timeoutSettings.setDefaultTimeout(timeout);
  }
  ...
  _didClose() {
    this.emit(events.Events.AndroidDevice.Close, this);
  }
  async shell(command) {
  ...
  async launchBrowser(options = {}) {
    ...
    context.once(events.Events.BrowserContext.Close, () => selectors._contextsForSelectors.delete(context));
  ...
  async waitForEvent(event, optionsOrPredicate = {}) {
    return await this._wrapApiCall(async () => {
      ...
      if (event !== events.Events.AndroidDevice.Close)
        waiter$1.rejectOnEvent(this, events.Events.AndroidDevice.Close, () => new errors.TargetClosedError());
...
class AndroidSocket extends channelOwner.ChannelOwner {
  static from(androidDevice) {
    return androidDevice._object;
  }
  constructor(parent, type, guid, initializer) {
    super(parent, type, guid, initializer);
    this._channel.on("data", ({ data }) => this.emit(events.Events.AndroidSocket.Data, data));
    this._channel.on("close", () => this.emit(events.Events.AndroidSocket.Close));
  }
  async write(data) {
...
class AndroidWebView extends eventEmitter.EventEmitter {
  ...
  async _fetchPage() {
    ...
    return browserContext.BrowserContext.from(context).pages()[0];
  }
}
