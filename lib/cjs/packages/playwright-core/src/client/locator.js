...
class Locator {
  constructor(frame, selector, options) {
    ...
    if (options?.hasText)
      this._selector += ` >> internal:has-text=${stringUtils.escapeForTextSelector(options.hasText, false)}`;
    if (options?.hasNotText)
      this._selector += ` >> internal:has-not-text=${stringUtils.escapeForTextSelector(options.hasNotText, false)}`;
    if (options?.has) {
  ...
  async _withElement(task, options) {
    ...
    const deadline = timeout ? time.monotonicTime() + timeout : 0;
    return await this._frame._wrapApiCall(async () => {
      ...
      const handle = elementHandle.ElementHandle.fromNullable(result.element);
      if (!handle)
        throw new Error(`Could not resolve ${this._selector} to DOM Element`);
      try {
        return await task(handle, deadline ? deadline - time.monotonicTime() : 0);
      } finally {
  ...
  locator(selectorOrLocator, options) {
    if (stringUtils.isString(selectorOrLocator))
  ...
  getByTestId(testId) {
    return this.locator(locatorUtils.getByTestIdSelector(testIdAttributeName(), testId));
  }
  ...
  toString() {
    return locatorGenerators.asLocator("javascript", this._selector);
  }
}
class FrameLocator {
