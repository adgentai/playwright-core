...
const multimap = require('../utils/isomorphic/multimap.js');
...
class Request extends channelOwner.ChannelOwner {
  ...
  frame() {
    if (!this._initializer.frame) {
      assert.assert(this.serviceWorker());
  ...
  _safePage() {
    return frame.Frame.fromNullable(this._initializer.frame)?._page || null;
  }
  serviceWorker() {
    return this._initializer.serviceWorker ? worker.Worker.from(this._initializer.serviceWorker) : null;
  }
  isNavigationRequest() {
  ...
  _applyFallbackOverrides(overrides) {
    ...
    if (stringUtils.isString(overrides.postData))
  ...
  _targetClosedScope() {
    return this.serviceWorker()?._closedScope || this._safePage()?._closedOrCrashedScope || new manualPromise.LongStandingScope();
  }
}
class Route extends channelOwner.ChannelOwner {
  constructor(parent, type, guid, initializer) {
    super(parent, type, guid, initializer);
  ...
  async _startHandling() {
    this._handlingPromise = new manualPromise.ManualPromise();
  ...
  async _innerFulfill(options = {}) {
    ...
    if (options.json !== void 0) {
      assert.assert(options.body === void 0, "Can specify either body or json parameters");
      body = JSON.stringify(options.json);
    }
    if (options.response instanceof fetch.APIResponse) {
    ...
    } else if (stringUtils.isString(body)) {
      isBase64 = false;
    ...
    for (const header of Object.keys(headersOption || {}))
      headers$1[header.toLowerCase()] = String(headersOption[header]);
    if (options.contentType)
      headers$1["content-type"] = String(options.contentType);
    else if (options.json)
      headers$1["content-type"] = "application/json";
    else if (options.path)
      headers$1["content-type"] = mimeType.getMimeTypeForPath(options.path) || "application/octet-stream";
    if (length && !("content-length" in headers$1))
      headers$1["content-length"] = String(length);
    await this._raceWithTargetClose(this._channel.fulfill({
      status: statusOption || 200,
      headers: headers.headersObjectToArray(headers$1),
      body,
      isBase64,
      fetchResponseUid
    }));
  ...
  async _innerContinue(isFallback) {
    ...
    return await this._raceWithTargetClose(this._channel.continue({
      ...
      headers: options.headers ? headers.headersObjectToArray(options.headers) : void 0,
      postData: options.postDataBuffer,
      isFallback
    }));
  }
}
class WebSocketRoute extends channelOwner.ChannelOwner {
  constructor(parent, type, guid, initializer) {
    super(parent, type, guid, initializer);
...
class WebSocketRouteHandler {
  ...
  static prepareInterceptionPatterns(handlers) {
    ...
    for (const handler of handlers) {
      if (stringUtils.isString(handler.url))
        patterns.push({ glob: handler.url });
      else if (rtti.isRegExp(handler.url))
  ...
  matches(wsURL) {
    return urlMatch.urlMatches(this._baseURL, wsURL, this.url, true);
  }
...
class Response extends channelOwner.ChannelOwner {
  constructor(parent, type, guid, initializer) {
    super(parent, type, guid, initializer);
...
class WebSocket extends channelOwner.ChannelOwner {
  ...
  constructor(parent, type, guid, initializer) {
    ...
    this._channel.on("frameSent", (event) => {
      if (event.opcode === 1)
        this.emit(events.Events.WebSocket.FrameSent, { payload: event.data });
      else if (event.opcode === 2)
        this.emit(events.Events.WebSocket.FrameSent, { payload: Buffer.from(event.data, "base64") });
    });
    this._channel.on("frameReceived", (event) => {
      if (event.opcode === 1)
        this.emit(events.Events.WebSocket.FrameReceived, { payload: event.data });
      else if (event.opcode === 2)
        this.emit(events.Events.WebSocket.FrameReceived, { payload: Buffer.from(event.data, "base64") });
    });
    this._channel.on("socketError", ({ error }) => this.emit(events.Events.WebSocket.Error, error));
    this._channel.on("close", () => {
      this._isClosed = true;
      this.emit(events.Events.WebSocket.Close, this);
    });
  }
  url() {
    return this._initializer.url;
  ...
  async waitForEvent(event, optionsOrPredicate = {}) {
    return await this._wrapApiCall(async () => {
      ...
      if (event !== events.Events.WebSocket.Error)
        waiter$1.rejectOnEvent(this, events.Events.WebSocket.Error, new Error("Socket error"));
      if (event !== events.Events.WebSocket.Close)
        waiter$1.rejectOnEvent(this, events.Events.WebSocket.Close, new Error("Socket closed"));
...
function validateHeaders(headers) {
  for (const key of Object.keys(headers)) {
    ...
    if (!Object.is(value, void 0) && !stringUtils.isString(value))
...
class RouteHandler {
  ...
  matches(requestURL) {
    return urlMatch.urlMatches(this._baseURL, requestURL, this.url);
  }
  async handle(route) {
    return await this._savedZone.run(async () => this._handleImpl(route));
  }
  async _handleImpl(route) {
    const handlerInvocation = { complete: new manualPromise.ManualPromise(), route };
    ...
    try {
      ...
      if (errors.isTargetClosedError(e)) {
        stackTrace.rewriteErrorMessage(e, `"${e.message}" while running route callback.
...
class RawHeaders {
  constructor(headers) {
    this._headersMap = new multimap.MultiMap();
