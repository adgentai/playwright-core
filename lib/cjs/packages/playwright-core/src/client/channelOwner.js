...
const clientStackTrace = require('./clientStackTrace.js');
const stackTrace = require('../utils/isomorphic/stackTrace.js');
const apiCallZone = require('../../../../cloudflare/apiCallZone.js');
const validatorPrimitives = require('../protocol/validatorPrimitives.js');

class ChannelOwner extends eventEmitter.EventEmitter {
  constructor(parent, type, guid, initializer) {
    ...
    this._channel = this._createChannel(new eventEmitter.EventEmitter(connection._platform));
    ...
    const channel = new Proxy(base, {
      get: (obj, prop) => {
        if (typeof prop === "string") {
          const validator = validatorPrimitives.maybeFindValidator(this._type, prop, "Params");
          const { internal } = protocolMetainfo.methodMetainfo.get(this._type + "." + prop) || {};
          if (validator) {
            return async (params) => {
  ...
  async _wrapApiCall(func, options) {
    ...
    const apiCall = apiCallZone.apiCallZone.getStore();
    const stackTrace$1 = clientStackTrace.captureLibraryStackTrace(this._platform);
    const apiZone = { title: options?.title, apiName: apiCall?.apiName ?? stackTrace$1.apiName, frames: stackTrace$1.frames, internal: options?.internal ?? false, reported: false, userData: void 0, stepId: void 0 };
    try {
      ...
      const stackFrames = "\n" + stackTrace.stringifyStackFrames(stackTrace$1.frames).join("\n") + innerError;
      if (stackFrames.trim())
