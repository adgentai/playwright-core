...
import { Response, statusText, Request } from '../network.js';
import { Network } from './third_party/bidiProtocolCore.js';
...
class BidiNetworkManager {
  ...
  _onResponseStarted(params) {
    ...
    const response = new Response(request.request, params.response.status, params.response.statusText, fromBidiHeaders(params.response.headers), timing, getResponseBody, false);
  ...
  async _updateProtocolRequestInterception(initial) {
    ...
    if (enabled) {
      interceptPromise = this._session.send("network.addIntercept", {
        phases: [Network.InterceptPhase.AuthRequired, Network.InterceptPhase.BeforeRequestSent],
...
export { BidiNetworkManager, bidiBytesValueToString };
