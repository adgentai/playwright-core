...
import { NavigationAbortedError } from '../frames.js';
import { helper } from '../helper.js';
import { mergeHeaders } from '../network.js';
import { Page, Worker, PageBinding } from '../page.js';
import { registry } from '../registry/index.js';
import { getAccessibilityTree } from './crAccessibility.js';
import { CRBrowserContext } from './crBrowser.js';
import { CRCoverage } from './crCoverage.js';
import { DragManager } from './crDragDrop.js';
import { CRExecutionContext, createHandle } from './crExecutionContext.js';
import { RawKeyboardImpl, RawMouseImpl, RawTouchscreenImpl } from './crInput.js';
import { CRNetworkManager } from './crNetworkManager.js';
import { CRPDF } from './crPdf.js';
import { toConsoleMessageLocation, exceptionToError, releaseObject } from './crProtocolHelper.js';
import { platformToFontFamilies } from './defaultFontFamilies.js';
...
class FrameSession {
  ...
  async _navigate(frame, url, referrer) {
    ...
    if (response.errorText)
      throw new NavigationAbortedError(response.loaderId, `${response.errorText} at ${url}`);
  ...
  _onAttachedToTarget(event) {
    ...
    session.once("Runtime.executionContextCreated", async (event2) => {
      worker.createExecutionContext(new CRExecutionContext(session, event2.context));
    });
