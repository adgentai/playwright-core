...
import { headersArrayToObject } from '../../utils/isomorphic/headers.js';
import { splitErrorMessage } from '../../utils/isomorphic/stackTrace.js';
import '../../../../../_virtual/pixelmatch.js';
import { jpegjs, PNG } from '../../utilsBundle.js';
...
import '../../zipBundle.js';
import { hostPlatform } from '../utils/hostPlatform.js';
import 'node:http';
import 'node:http2';
import 'node:https';
import 'node:url';
...
import { Page, PageBinding } from '../page.js';
import { getAccessibilityTree } from './wkAccessibility.js';
import { WKSession } from './wkConnection.js';
import { WKExecutionContext, createHandle } from './wkExecutionContext.js';
import { RawKeyboardImpl, RawMouseImpl, RawTouchscreenImpl } from './wkInput.js';
import { WKInterceptableRequest, WKRouteImpl } from './wkInterceptableRequest.js';
import { WKProvisionalPage } from './wkProvisionalPage.js';
import { WKWorkers } from './wkWorkers.js';
...
class WKPage {
  ...
  async _initializeSessionMayThrow(session, resourceTreeHandler) {
    ...
    promises.push(session.send("Network.setExtraHTTPHeaders", { headers: headersArrayToObject(
  ...
  async _onTargetCreated(event) {
    ...
    const session = new WKSession(this._pageProxySession.connection, targetInfo.targetId, (message) => {
  ...
  async adoptElementHandle(handle, to) {
    ...
    return createHandle(to, result.object);
  }
  ...
  async getFrameElement(frame) {
    ...
    if (!result || result.object.subtype === "null")
      throw new Error("Frame has been detached.");
    return createHandle(context, result.object);
  }
