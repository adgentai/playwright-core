...
import { isNonRecoverableDOMError, assertDone, throwElementIsNotAttached } from './dom.js';
import { TimeoutError } from './errors.js';
import { prepareFilesForUpload } from './fileUploadUtils.js';
import { FrameSelectors } from './frameSelectors.js';
import { helper } from './helper.js';
import { SdkObject, serverSideCallMetadata } from './instrumentation.js';
import { ExecutionContext, isJavaScriptErrorInEvaluate, normalizeEvaluationExpression } from './javascript.js';
import { Route, WebSocket, parseURL } from './network.js';
import { Page } from './page.js';
import { ProgressController, isAbortError } from './progress.js';
...
import { LongStandingScope, ManualPromise } from '../utils/isomorphic/manualPromise.js';
import { renderTitleForCall } from '../utils/isomorphic/protocolFormatter.js';
import { constructURLBasedOnBaseURL } from '../utils/isomorphic/urlMatch.js';
...
import { isInvalidSelectorError } from '../utils/isomorphic/cssParser.js';
...
class FrameManager {
  ...
  requestStarted(request, route) {
    ...
    if (route)
      new Route(request, route).handle([...this._page.requestInterceptors, ...this._page.browserContext.requestInterceptors]);
  }
  ...
  onWebSocketCreated(requestId, url) {
    const ws = new WebSocket(this._page, url);
...
class Frame extends SdkObject {
  ...
  async gotoImpl(progress, url, options) {
    ...
    if (!this._firedLifecycleEvents.has(waitUntil))
      await helper.waitForEvent(progress, this, Frame.Events.AddLifecycle, (e) => e === waitUntil).promise;
  ...
  async _waitForNavigation(progress, requiresNewDocument, options) {
    ...
    const navigationEvent = await helper.waitForEvent(progress, this, Frame.Events.InternalNavigation, (event) => {
      if (event.error)
        return true;
  ...
  async click(progress, selector, options) {
    return assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, !options.force, (handle) => handle._click(progress, { ...options, waitAfter: !options.noWaitAfter })));
  }
  async dblclick(progress, selector, options) {
    return assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, !options.force, (handle) => handle._dblclick(progress, options)));
  }
  async dragAndDrop(progress, source, target, options) {
    assertDone(await this._retryWithProgressIfNotConnected(progress, source, options.strict, !options.force, async (handle) => {
    ...
    assertDone(await this._retryWithProgressIfNotConnected(progress, target, options.strict, false, async (handle) => {
  ...
  async tap(progress, selector, options) {
    ...
    return assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, !options.force, (handle) => handle._tap(progress, options)));
  }
  async fill(progress, selector, value, options) {
    return assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, !options.force, (handle) => handle._fill(progress, value, options)));
  }
  async focus(progress, selector, options) {
    assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, true, (handle) => handle._focus(progress)));
  }
  async blur(progress, selector, options) {
    assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, true, (handle) => handle._blur(progress)));
  }
  ...
  async hover(progress, selector, options) {
    return assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, !options.force, (handle) => handle._hover(progress, options)));
  }
  ...
  async setInputFiles(progress, selector, params) {
    ...
    return assertDone(await this._retryWithProgressIfNotConnected(progress, selector, params.strict, true, (handle) => handle._setInputFiles(progress, inputFileItems)));
  }
  async type(progress, selector, text, options) {
    return assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, true, (handle) => handle._type(progress, text, options)));
  }
  async press(progress, selector, key, options) {
    return assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, true, (handle) => handle._press(progress, key, options)));
  }
  async check(progress, selector, options) {
    return assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, !options.force, (handle) => handle._setChecked(progress, true, options)));
  }
  async uncheck(progress, selector, options) {
    return assertDone(await this._retryWithProgressIfNotConnected(progress, selector, options.strict, !options.force, (handle) => handle._setChecked(progress, false, options)));
  }
  ...
  async _expectImpl(progress, selector, options, errorResultHandler) {
    ...
    const handleError = (e, isProgressError) => {
      if (isJavaScriptErrorInEvaluate(e) || isInvalidSelectorError(e))
        throw e;
      const result = { matches: options.isNot, log: compressCallLog(progress.metadata.log) };
...
export { Frame, FrameManager, NavigationAbortedError };
