...
import { ExecutionContext, evaluate, evaluateExpression, JSHandle } from './javascript.js';
...
import { source } from '../generated/injectedScriptSource.js';
...
class FrameExecutionContext extends ExecutionContext {
  ...
  async evaluateExpression(expression, options, arg) {
    return evaluateExpression(this, expression, { ...options, returnByValue: true }, arg);
  }
  async evaluateExpressionHandle(expression, options, arg) {
    return evaluateExpression(this, expression, { ...options, returnByValue: false }, arg);
  }
  injectedScript() {
...
class ElementHandle extends JSHandle {
  ...
  async _setInputFiles(progress, items) {
    ...
    if (localPaths || localDirectory) {
      ...
      await progress.race(Promise.all(localPathsOrDirectory.map((localPath) => fs.promises.access(localPath, fs.constants.F_OK))));
...
export { ElementHandle, FrameExecutionContext, NonRecoverableDOMError, assertDone, isNonRecoverableDOMError, kUnableToAdoptErrorMessage, throwElementIsNotAttached, throwRetargetableDOMError };
