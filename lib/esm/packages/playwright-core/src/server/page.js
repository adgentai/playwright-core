...
import { FrameManager, Frame } from './frames.js';
import { helper } from './helper.js';
import { Keyboard, Mouse, Touchscreen } from './input.js';
import { SdkObject } from './instrumentation.js';
import { isJavaScriptErrorInEvaluate, ExecutionContext, evaluateExpression } from './javascript.js';
import { Screenshotter, validateScreenshotOptions } from './screenshotter.js';
import { assert } from '../utils/isomorphic/assert.js';
import { asLocator } from '../utils/isomorphic/locatorGenerators.js';
import { ManualPromise, LongStandingScope } from '../utils/isomorphic/manualPromise.js';
import { renderTitleForCall } from '../utils/isomorphic/protocolFormatter.js';
import { trimStringWithEllipsis } from '../utils/isomorphic/stringUtils.js';
import { getComparator } from './utils/comparators.js';
import 'node:crypto';
...
import { source } from '../generated/bindingsControllerSource.js';
...
class Page extends SdkObject {
  ...
  async reportAsNew(opener, error = void 0, contextEvent = BrowserContext.Events.Page) {
    if (opener) {
  ...
  _markInitialized(error = void 0, contextEvent = BrowserContext.Events.Page) {
    if (error) {
...
`).join("");
  ...
  async _performWaitForNavigationCheck(progress) {
    ...
    await helper.waitForEvent(progress, mainFrame, Frame.Events.InternalNavigation, (e) => {
      if (!e.isPublic)
        return false;
      if (!e.error)
...
class Worker extends SdkObject {
...
export { InitScript, Page, PageBinding, Worker };
